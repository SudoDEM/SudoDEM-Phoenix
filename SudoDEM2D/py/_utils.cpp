...
pybind11::tuple Shop__getStressAndTangent2D(Real z_dim=1, bool symmetry=true){return Shop::getStressAndTangent2D(z_dim,symmetry);}
pybind11::tuple Shop__getStressTangentThermal2D(Real z_dim=1, bool symmetry=true){return Shop::getStressTangentThermal2D(z_dim,symmetry);}
...
void pybind_init__utils(pybind11::module& m){
	...
	m.def("unbalancedForce",&Shop__unbalancedForce,pybind11::arg("useMaxForce")=false,"Compute the ratio of mean (or maximum, if *useMaxForce*) summary force on bodies and mean force magnitude on interactions. For perfectly static equilibrium, summary force on all bodies is zero (since forces from interactions cancel out and induce no acceleration of particles); this ratio will tend to zero as simulation stabilizes, though zero is never reached because of finite precision computation. Sufficiently small value can be e.g. 1e-2 or smaller, depending on how much equilibrium it should be.");
	m.def("getParticleVolume2D",Shop__getParticlesVolume2D,"Compute the total volume (area for 2D) of particles in the scene.");
	m.def("getMeanCN",Shop__meanCoordinationNumber,"Get the mean coordination number of a packing.");
	m.def("changeParticleSize2D",Shop__changeParticleSize2D,"expand a particle by a given coefficient");
	m.def("getVoidRatio2D", Shop__getVoidRatio2D, pybind11::arg("cellArea")=1,"Compute 2D void ratio. Keyword cellArea is effective only for aperioidc cell.");
	m.def("getStress2D",Shop__getStress2D,pybind11::arg("z_dim")=1,"Compute overall stress of periodic cell.");
	m.def("getFabricTensorCN2D",Shop__getFabricTensorCN2D,"Fabric tensor of contact normal.");
	m.def("getFabricTensorPO2D",Shop__getFabricTensorPO2D,"Fabric tensor of particle orientation along the rx axis.");
	m.def("getStressAndTangent2D",Shop__getStressAndTangent2D,pybind11::arg("z_dim")=1,pybind11::arg("symmetry")=true,"Compute overall stress of periodic cell using the same equation as function getStress. In addition, the tangent operator is calculated using the equation published in [Kruyt and Rothenburg1998]_:\n\n.. math:: S_{ijkl}=\\frac{1}{V}\\sum_{c}(k_n n_i l_j n_k l_l + k_t t_i l_j t_k l_l)\n\n:param float volume: same as in function getStress\n:param bool symmetry: make the tensors symmetric.\n\n:return: macroscopic stress tensor and tangent operator as py::tuple");
	m.def("getStressTangentThermal2D",Shop__getStressTangentThermal2D,pybind11::arg("z_dim")=1,pybind11::arg("symmetry")=true,"Compute overall stress of periodic cell using the same equation as function getStress. In addition, the tangent operator is calculated using the equation published in [Kruyt and Rothenburg1998]_:\n\n.. math:: S_{ijkl}=\\frac{1}{V}\\sum_{c}(k_n n_i l_j n_k l_l + k_t t_i l_j t_k l_l)\n\n:param float volume: same as in function getStress\n:param bool symmetry: make the tensors symmetric. Finally, the thermal conductivity tensor is calculated based on the formular in PFC.\n\n:return: macroscopic stress tensor and tangent operator as py::tuple");
	#ifdef SUDODEM_OPENGL
