...
pybind11::tuple bestFitOBB_py(const pybind11::tuple& _pts){
	int l=pybind11::len(_pts);
	if(l<=1) throw std::runtime_error("Cloud must have at least 2 points.");
	std::vector<Vector3r> pts; pts.resize(l);
	for(int i=0; i<l; i++) pts[i]=_pts[i].cast<Vector3r>();
	Quaternionr rot; Vector3r halfSize, center;
	bestFitOBB(pts,center,halfSize,rot);
	return pybind11::make_tuple(center,halfSize,rot);
}

void pybind_init__packObb(pybind11::module& m){
	m.attr("__doc__")="Computation of oriented bounding box for cloud of points.";
	m.def("cloudBestFitOBB",&bestFitOBB_py,"Return (Vector3 center, Vector3 halfSize, Quaternion orientation) of\nbest-fit oriented bounding-box for given tuple of points\n(uses brute-force velome minimization, do not use for very large clouds).");
};
