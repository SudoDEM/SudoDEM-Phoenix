...
class ScGeom: public IGeom {//for 2D
	...
	public:
		...
		ScGeom() : refR1(0), refR2(0), penetrationDepth(NaN), shearInc(Vector2r::Zero()), radius1(refR1), radius2(refR2) {
			createIndex();
			preNormal = Vector2r::Zero();
		}
		
		// Use REGISTER_ATTRIBUTES for serialization
		REGISTER_ATTRIBUTES(IGeom, normal, contactPoint, refR1, refR2, penetrationDepth, shearInc);
		...
		virtual void pyRegisterClass(pybind11::module_ _module) override {
			...
			_classObj.def_readwrite("contactPoint", &ScGeom::contactPoint, "some reference point for the interaction (usually in the middle). |ycomp|");
			_classObj.def_readwrite("refR1", &ScGeom::refR1, "Reference radius of particle #1. |ycomp|");
			_classObj.def_readwrite("refR2", &ScGeom::refR2, "Reference radius of particle #2. |ycomp|");
			_classObj.def_readwrite("penetrationDepth", &ScGeom::penetrationDepth, "Penetration distance of disks (positive if overlapping)");
