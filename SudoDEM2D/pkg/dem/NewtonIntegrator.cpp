...
void NewtonIntegrator::action()
{
	...
	const bool isPeriodic(scene->isPeriodic);
		...
		for (Real& thrMaxVSq : threadMaxVelocitySq) { thrMaxVSq=0; }
		#endif
			for (const shared_ptr<Body>& b : *scene->bodies){			// clump members are handled inside clumps
			...
			#ifdef SUDODEM_DEBUG
				if(isnan(f[0])||isnan(f[1]) throw runtime_error(("NewtonIntegrator: NaN force acting on #"+std::to_string(id)+".").c_str());
				if(isnan(m)) throw runtime_error(("NewtonIntegrator: NaN torque acting on #"+std::to_string(id)+".").c_str());
