...
void PeriTriaxController::pyRegisterClass(pybind11::module_ _module)
{
	...
	_classObj.def(pybind11::init([](bool dynCellin, Real zdim_in, Vector2r goalin, 
		                        int maskin, Real relstresstolin, Real maxbain,
								Vector2r maxrate,
								std::string donehookin, std::string labelin) {
		auto engine = std::make_shared<PeriTriaxController>();
		engine->dynCell = dynCellin;
		engine->z_dim = zdim_in;

		engine->goal = goalin;

		engine->stressMask = maskin;

		engine->relStressTol = relstresstolin;
		engine->maxUnbalanced = maxbain;
		engine->maxStrainRate = maxrate;
		engine->doneHook = donehookin;
		engine->label = labelin;

		return engine;
	}), pybind11::arg("dynCell")=true, pybind11::arg("z_dim")=0.05, pybind11::arg("goal")=Vector2r(-1e5,-1e5),
	   pybind11::arg("stressMask")=0, pybind11::arg("relStressTol")=0.001,
	   pybind11::arg("maxUnbalanced")=0.001, pybind11::arg("maxStrainRate")=Vector2r(0.5,0.5),
	   pybind11::arg("doneHook")="",
	   pybind11::arg("label")="biax");
