...
bool Ig2_Superellipse_Superellipse_SuperellipseGeom::go(const shared_ptr<Shape>& cm1,const shared_ptr<Shape>& cm2,const State& state1,const State& state2, const Vector2r& shift2, const bool& force, const shared_ptr<Interaction>& interaction){
	...
	if (isNew) {
        ...
        //cout<<"para0="<<para<<endl;
            ...
            //to do ...
        #endif
        // std::cerr << "DEBUG Ig2: after _USE_LM_OPTIMIZATION block" << std::endl;
	}else{
    ...
    // std::cerr << "DEBUG Ig2: after if/else block, bang=" << bang << std::endl;
	Vector2r contactNormal(0,0), center(0,0);
    // std::cerr << "DEBUG Ig2: initialized contactNormal and center" << std::endl;
	double pDepth(0.0);
	Vector2r p1,p2;
	// std::cerr << "DEBUG Ig2: checking isSphere, A->isSphere=" << A->isSphere << " B->isSphere=" << B->isSphere << std::endl;
	...
	// std::cerr << "DEBUG Ig2: in non-spherical branch" << std::endl;
	//contact detection using bounding spheres
        double r_sum = A->getr_max()+B->getr_max();
        // std::cerr << "DEBUG Ig2: r_sum=" << r_sum << std::endl;

        if ((position1 - position2).norm() > r_sum){//no contact
                // std::cerr << "DEBUG Ig2: no contact (bounding sphere)" << std::endl;
                bang->PenetrationDepth=0;
                bang->isShearNew = true;
                return true;
        }
        // std::cerr << "DEBUG Ig2: passed bounding sphere check" << std::endl;
        // std::cerr << "DEBUG Ig2: about to call lmder2" << std::endl;
        ...
        // std::cerr << "DEBUG Ig2: lmder2 returned" << std::endl;
        // std::cerr << "DEBUG Ig2: para=" << para << " touching=" << touching << std::endl;
        ...
        // std::cerr << "DEBUG Ig2: after nan check" << std::endl;
        //cout<<"para1="<<para<<endl;
				pDepth = (p1-p2).dot(contactNormal);
        // std::cerr << "DEBUG Ig2: pDepth computed" << std::endl;
        if(touching){
            // std::cerr << "DEBUG Ig2: in touching block" << std::endl;
            ...
            if(10.0*(p2-p1).norm() > A->getr_max()){//the contact depth is tremendously large which might be at local minimia. The common contact depth should be 3 order of magnitude less than particle size.FIXME:the coefficient of 10.0 may be not the best try.
                // std::cerr << "DEBUG Ig2: checking isInside" << std::endl;
        ...
        // std::cerr << "DEBUG Ig2: after touching block" << std::endl;
	    ...
	    if (!touching){
	            ...
	            bang->isShearNew = true;
	            return true;}
        // std::cerr << "DEBUG Ig2: after !touching check" << std::endl;
	    ...
	    //if((se22.position-centroid).dot(normal)<0) normal*=-1;
            // std::cerr << "DEBUG Ig2: about to set point1/point2" << std::endl;
            bang->point1 = p1;
            bang->point2 = p2;
            // std::cerr << "DEBUG Ig2: point1/point2 set" << std::endl;
        //test jacobian matrix ends
        //std::cerr << "watch 6"<< "\n";
	    //contactNormal = p1 - p2;
        // std::cerr << "DEBUG Ig2: about to check isnan contactNormal" << std::endl;
	    if(isnan(contactNormal[0])){
            // std::cerr << "DEBUG Ig2: contactNormal is NAN" << std::endl;
        ...
        // std::cerr << "DEBUG Ig2: after isnan check" << std::endl;


	        bang->contactPoint=center;
	        bang->PenetrationDepth= pDepth;//contactNormal.norm();
        // std::cerr << "DEBUG Ig2: contactPoint and PenetrationDepth set" << std::endl;
            ...
            #define GEOM_GL
            #ifdef GEOM_GL
            // std::cerr << "DEBUG Ig2: in GEOM_GL section" << std::endl;
            //cout<<"para="<<para<<" pos="<<position1<<endl;
            Vector2r contact(cos(para),sin(para));
            Real phi;
						//cout<<"p1 and p2="<<p1<<" "<<p2;
            // std::cerr << "DEBUG Ig2: about to call A->Normal2Phi" << std::endl;
	        phi= A->Normal2Phi(A->rot_mat2local*contact);
            // std::cerr << "DEBUG Ig2: phi=" << phi << std::endl;
            Vector2r n1,n2;
            //cout<<"phi="<<phi<<endl;
            n1 = A->getNormal(phi);
            ...
            // std::cerr << "DEBUG Ig2: GEOM_GL section done" << std::endl;
            //Vector2r tmp1;
            ...
            #endif
        // std::cerr << "DEBUG Ig2: about to normalize contactNormal" << std::endl;
        contactNormal.normalize();
        // std::cerr << "DEBUG Ig2: contactNormal normalized" << std::endl;
	    /*if (!touching){
	            bang->contactAngle = para;
	            bang->PenetrationDepth=0;
	            bang->isShearNew = true;
	...
	// std::cerr << "DEBUG Ig2: non-spherical block done" << std::endl;
	// std::cerr << "DEBUG Ig2: about to call precompute" << std::endl;
	// Ensure scene pointer is valid
	Scene* scenePtr = scene;
	if (!scenePtr) {
		// std::cerr << "DEBUG Ig2: scene is null, getting from Omega" << std::endl;
		scenePtr = Omega::instance().getScene().get();
	}
	bang->precompute(state1,state2,scenePtr,interaction,contactNormal,bang->isShearNew,shift2);
	// std::cerr << "DEBUG Ig2: precompute done" << std::endl;
...
bool Ig2_Wall_Superellipse_SuperellipseGeom::go(const shared_ptr<Shape>& cm1,const shared_ptr<Shape>& cm2,const State& state1,const State& state2, const Vector2r& shift2, const bool& force, const shared_ptr<Interaction>& interaction){
  ...
  //cout<<"is Sphere? "<<B->isSphere<<endl;
	...
	Scene* scenePtr = scene;
	if (!scenePtr) {
		// std::cerr << "DEBUG Ig2_Wall: scene is null, getting from Omega" << std::endl;
		scenePtr = Omega::instance().getScene().get();
	}
	bang->precompute(state1,state2,scenePtr,interaction,normal,bang->isShearNew,shift2);
	bang->normal= normal;
	//bang->contactAngle = para;
							...
							const shared_ptr<Interaction>& interaction)
{
	...
	Scene* scenePtr = scene;
	if (!scenePtr) {
		// std::cerr << "DEBUG Ig2_Fwall: scene is null, getting from Omega" << std::endl;
		scenePtr = Omega::instance().getScene().get();
	}
	bang->precompute(state1,state2,scenePtr,interaction,normal,bang->isShearNew,shift2);
	bang->normal= normal;
