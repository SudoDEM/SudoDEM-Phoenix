...
double gammln(const double xx) {
	...
	if (xx <= 0) throw std::runtime_error("bad arg in gammln: xx=" + std::to_string(xx) + " <= 0");
	y=x=xx;
...
{       //cout<<"watch init"<<init<<endl;
        if (init) return;
        // Validate eps - must be positive for beta function
        if (eps <= 0) {
            LOG_WARN("Superellipse::Initial(): Invalid eps=" << eps << ", setting to default 1.0");
            eps = 1.0;
        }
        //
        rx = rxy(0);
...
/* Plotting */
	...
	bool Gl1_Superellipse::wire = false;

	int  Gl1_Superellipse::Slices = 12;
	...
	void Gl1_Superellipse::initSolidGlList(Superellipse* t) {
		...
		if (glIsList(t->m_glSolidListNum) == GL_TRUE) {
	...
	void Gl1_Superellipse::initWireGlList(Superellipse* t){
		...
		if (glIsList(t->m_glWireListNum) == GL_TRUE) {
...
currentNormal, bool isNew, const Vector2r& shift){
    ...
    preNormal = normal;
		...
		// Debug: check scene and c pointers
		//std::cerr << "DEBUG SuperellipseGeom::precompute: scene=" << scene << " c=" << c << std::endl;
		// Debug: check currentNormal for NaN
		//std::cerr << "DEBUG SuperellipseGeom::precompute: currentNormal=[" << currentNormal[0] << "," << currentNormal[1] << "] isnan0=" << std::isnan(currentNormal[0]) << " isnan1=" << std::isnan(currentNormal[1]) << std::endl;
