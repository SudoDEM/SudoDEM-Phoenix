...
*  These macros are kept for backward compatibility but are now no-ops.
*************************************************************************/
...
// Single class plugin registration - creates static registrar
// NOTE: This is now a no-op. Manual registration is done in ClassRegistryInstantiation.cpp and EngineClasses.cpp
// The static initialization approach was causing ordering issues across translation units
#define SUDODEM_PLUGIN_IMPL(ClassName) /* Manual registration in ClassRegistryInstantiation.cpp and EngineClasses.cpp */
...
// Single class plugin registration - now a no-op (backward compatibility)
#define SUDODEM_REGISTER_PLUGIN(ClassName) SUDODEM_PLUGIN_IMPL(ClassName)
