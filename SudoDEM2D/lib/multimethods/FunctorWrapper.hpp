...
///
/// class ShapeDraw : public FunctorWrapper< std::string , TYPELIST_4(std::shared_ptr<Shape>,double,char,const std::string) >
/// {}
///
/// then virtual function to overload is:
///		public: virtual std::string go(std::shared_ptr<Shape>&,double,char,const std::string& );
///
...
class FunctorWrapper //: public Serializable // FIXME functor shouldn't be serializable
{
	private :
		...
		ResultType error(int n)
		{
			...
			+ "6. " + typeid(Parm6).name() + "\n"
			+ "7. " + typeid(Parm7).name() + "\n"
			+ "number of types used in the call: " + std::to_string(n) + "\n").c_str());
		}
...
#define DEFINE_FUNCTOR_ORDER_2D(class1,class2)							\
	public : virtual std::string checkOrder() const override				\
	{											\
