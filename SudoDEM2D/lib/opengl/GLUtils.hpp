...
struct GLUtils{
	...
	static void GLDrawInt(long i, const Vector2r& pos, const Vector3r& color=Vector3r(1,1,1)){
		GLUtils::GLDrawText(std::to_string(i),pos,color);
	}

	static void GLDrawText(const std::string& txt, const Vector2r& pos, const Vector3r& color=Vector3r(1,1,1)){
#if QT_VERSION_MAJOR >= 6
		// Qt6: Skip text rendering for now (would need QPainter context)
		// Text rendering will be handled by the GLViewer class
#else
		glPushMatrix();
