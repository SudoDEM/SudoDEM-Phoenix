...
void Controller::setupDisplayTab()
{
    ...
    // Display preset combo for shape functors - simple styling
...
void Controller::refreshDisplayCombo()
{
    ...
    // Try to dynamically populate from ClassRegistry, with fallback to hardcoded list
    ...
    try {
        ...
        // Check if registry has any classes registered
        if (registry.size() == 0) {
            throw std::runtime_error("ClassRegistry not initialized");
        }
        ...
        for (const auto& className : allClasses) {
            checkedCount++;
            try {
                bool inherits = registry.isInheritingFrom(className, glShapeFunctorTypeName);
                if (inherits) {
                ...
                // Skip classes that cause issues during inheritance check
                continue;
            }
        }
        ...
        // Failed to populate dynamically, will use fallback
    } catch (...) {
        // Unknown error, will use fallback
    }

    // Fallback to hardcoded list if dynamic population failed
    if (!dynamicSuccess) {
        m_displayCombo->addItem("Gl1_Wall");
        m_displayCombo->addItem("Gl1_Superellipse");
        m_displayCombo->addItem("Gl1_Disk");
        m_displayCombo->addItem("Gl1_Fwall");
    }
}
            ...
            if (numProps > 0) {
                ...
                for (Py_ssize_t i = 0; i < numProps; i++) {
                    ...
                    if (type == "bool") {
                        ...
                        connect(checkBox, &QCheckBox::stateChanged, [this, propName, functorName](int state) {
                            PyGILState_STATE gstate = PyGILState_Ensure();
                            std::string code = "import __main__\n"
                                "if hasattr(__main__, '_current_display_functor'):\n"
                                "    prop = '" + propName.toStdString() + "'\n"
                                "    val = " + (state ? "True" : "False") + "\n"
                                "    # Set on the current functor\n"
                                "    if __main__._display_prop_is_static.get(prop, False):\n"
                                "        setattr(__main__._current_display_functor.__class__, prop, val)\n"
                                "    else:\n"
                                "        setattr(__main__._current_display_functor, prop, val)\n"
                                "    # Also update the actual functor in the renderer\n"
                                "    if hasattr(O, 'renderer') and O.renderer and hasattr(O.renderer, 'shapeDispatcher'):\n"
                                "        for cb in O.renderer.shapeDispatcher.functors:\n"
                                "            if cb.__class__.__name__ == '" + functorName.toStdString() + "':\n"
                                "                setattr(cb, prop, val)\n";
                        ...
                        connect(spinBox, QOverload<int>::of(&QSpinBox::valueChanged),
                                [this, propName, functorName](int value) {
                            PyGILState_STATE gstate = PyGILState_Ensure();
                            std::string code = "import __main__\n"
                                "if hasattr(__main__, '_current_display_functor'):\n"
                                "    prop = '" + propName.toStdString() + "'\n"
                        ...
                        connect(spinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
                                [this, propName, functorName](double value) {
                            PyGILState_STATE gstate = PyGILState_Ensure();
                            std::string code = "import __main__\n"
                                "if hasattr(__main__, '_current_display_functor'):\n"
                                "    prop = '" + propName.toStdString() + "'\n"
