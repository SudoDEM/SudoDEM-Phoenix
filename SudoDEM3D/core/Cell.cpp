...
void Cell::integrateAndUpdate(Real dt){
	...
	_hasShear=(hSize(0,1)!=0 || hSize(1,0)!=0 || hSize(0,2)!=0 || hSize(2,0)!=0 || hSize(1,2)!=0 || hSize(2,1)!=0);
...
void Cell::pyRegisterClass(pybind11::module_ _module) {
	...
	_classObj.def_property_readonly("shearTrsf", [](const Cell& c) -> const Matrix3r& { return c._shearTrsf; }, "Current skew+rot transformation (no resize)");
	_classObj.def_property_readonly("unshearTrsf", [](const Cell& c) -> const Matrix3r& { return c._unshearTrsf; }, "Inverse of the current skew+rot transformation (no resize)");
	_classObj.def_property_readonly("hSize0", [](const Cell& c) -> Matrix3r { return c.getHSize0(); }, "Value of untransformed hSize, with respect to current trsf (computed as trsf^{-1} × hSize.");
}
