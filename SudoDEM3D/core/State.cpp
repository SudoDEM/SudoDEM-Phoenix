...
// Helper to convert Python sequence to Vector3r
static Vector3r seqToVector3r(const pybind11::object& obj) {
    auto seq = obj.cast<pybind11::sequence>();
    if (pybind11::len(seq) != 3) throw std::runtime_error("Sequence must have 3 elements");
    return Vector3r(
        seq[0].cast<double>(),
        seq[1].cast<double>(),
        seq[2].cast<double>()
...
void State::setDOFfromVector3r(Vector3r disp_rot){
	blockedDOFs=((disp_rot[0]==1.0)?DOF_X :0)|((disp_rot[1]==1.0)?DOF_Y :0)|((disp_rot[2]==1.0)?DOF_Z :0)|((disp_rot[3]==1.0)?DOF_RX :0)|((disp_rot[4]==1.0)?DOF_RY :0)|((disp_rot[5]==1.0)?DOF_RZ :0);
}
...
void State::blockedDOFs_vec_set(const std::string& dofs){
	...
	for(char c : dofs){
		...
		#undef _GET_DOF
		throw std::invalid_argument("Invalid  DOF specification `"+std::to_string(c)+"' in '"+dofs+"', characters must be ∈{xyzXYZ}.");
	}
}
...
void State::pyRegisterClass(pybind11::module_ _module) {
	...
	// Register attributes
	_classObj.def_readwrite("se3", &State::se3, "Position and orientation as one object.");
	...
	_classObj.def_property("pos", 
		[](const State& s) -> Vector3r { return s.pos; },
		[](State& s, const pybind11::object& obj) { 
			// Try Eigen conversion first, fall back to manual conversion
			try {
				s.pos = obj.cast<Vector3r>();
			} catch (...) {
				s.pos = seqToVector3r(obj);
			}
		},
		"Current position.");
