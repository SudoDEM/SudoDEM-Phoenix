...
static bool sudodem_ensure_pip(const filesystem::path& pythonExe) 
{
    ...
    cout << "pip not found. Trying to bootstrap pip with ensurepip..." << endl;
      ...
      "Library" / "Application Support" / "SudoDEM3D" / "python3_13";
...
// Force OpenMP initialization BEFORE Qt6 loads to avoid threading race
// This runs as a static initializer, before main()
struct ForceOpenMPInit {
    ForceOpenMPInit() {
        // Set OMP_NUM_THREADS before any Qt6 code runs
...
static ForceOpenMPInit forceOpenMPInit;
...
int main( int argc, char **argv )
{
  ...
  if (argc > 0)
  {
    if (exeName.size() > 0)
    {
      ...
      // Normalize path to handle ./ and ../
          ...
          // std::cout<<"Pathnorm "<<exePathNorm<<std::endl;
          ...
          // std::cout<<"newpath "<<newPathValue<<std::endl;
          ...
          // std::cout<<"libpath "<<libPathValue<<std::endl;

          std::string libLDPathValue = (exePath.parent_path().parent_path() / "lib" / "3rdlibs").string();

          // std::cout<<"libLDpath "<<libLDPathValue<<std::endl;
  ...
  // Note: OMP_NUM_THREADS is now set in static initializer before main()
  // options
  ...
  Py_SetProgramName(program);
    ...
    std::filesystem::path(std::getenv("HOME")) / "Library" / "Application Support" / "SudoDEM3D" /
...
"def userSession():\n"
  ...
  if (useGUI) {
    ...
    g_qApp = &app;
    cout<<"QApplication created successfully"<<endl;

    // Initialize OpenGL manager
    cout<<"Creating OpenGLManager..."<<endl;
    OpenGLManager* glm = new OpenGLManager();
    cout<<"OpenGLManager created successfully"<<endl;
    ...
    cout<<"Creating GLViewer..."<<endl;
    int viewId = glm->waitForNewView(5.0, false); // Don't center yet
    cout<<"GLViewer created, viewId="<<viewId<<endl;
    if (viewId < 0) {
