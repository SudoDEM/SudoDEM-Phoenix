...
static_assert(sizeof(Vector3r)==3*sizeof(Real), "Vector3r must be contiguous in memory");
...
class ForceContainer{
	private:
		...
		typedef std::vector<Vector3r> vector;
		...
		friend class cereal::access; template<class ArchiveT> void serialize(ArchiveT & ar, unsigned int version){}
	public:
				ForceContainer(): size(0), permSize(0),syncedSizes(true),synced(true),moveRotUsed(false),permForceUsed(false),_zero(Vector3r::Zero()),_zeror(Vector3r::Zero()),syncCount(0),lastReset(0){
					nThreads=omp_get_max_threads();
					for(int i=0; i<nThreads; i++){				_forceData.push_back(vvector()); _torqueData.push_back(vector());
		...
		const Vector3r& getTorque(Body::id_t id)        { ensureSynced(); return ((size_t)id<size)?_torque[id]:_zeror; }
		...
		const Vector3r& getRot(Body::id_t id)           { ensureSynced(); return ((size_t)id<size)?_rot[id]:_zeror; }
		...
		const Vector3r& getPermTorque(Body::id_t id) { ensureSynced(); return ((size_t)id<size)?_permTorque[id]:_zeror; }
		...
		* for a particular body id. */
		...
		Vector3r getTorqueSingle(Body::id_t id){ Vector3r ret(Vector3r::Zero()); for(int t=0; t<nThreads; t++){ ret+=((size_t)id<sizeOfThreads[t])?_torqueData[t][id]:_zeror; } if (permForceUsed) ret+=_permTorque[id]; return ret; }
		...
		Vector3r getRotSingle   (Body::id_t id){ Vector3r ret(Vector3r::Zero()); for(int t=0; t<nThreads; t++){ ret+=((size_t)id<sizeOfThreads[t])?_rotData   [t][id]:_zeror; } return ret; }
		...
		inline void sync(){
			...
			for(long id=0; id<(long)size; id++){
				Vector3r sumF(Vector3r::Zero());Vector3r sumT(Vector3r::Zero());
			...
			if(moveRotUsed){
				for(long id=0; id<(long)size; id++){
					Vector3r sumM(Vector3r::Zero()); Vector3r sumR(Vector3r::Zero());
