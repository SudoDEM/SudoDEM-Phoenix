...
class Shop{
	public:
		...
		static vector<std::tuple<Vector3r,Real,int> > loadSpheresFromFile(const string& fname,Vector3r& minXYZ, Vector3r& maxXYZ, Vector3r* cellSize=NULL);
		...
		static std::tuple<Real, Real, Real> spiralProject(const Vector3r& pt, Real dH_dTheta, int axis=2, Real periodStart=std::numeric_limits<Real>::quiet_NaN(), Real theta0=0);
		...
		SUDODEM_PYREGISTER_CLASS_API static py::list getStressLWForEachBody();
		...
		SUDODEM_PYREGISTER_CLASS_API static py::tuple normalShearStressTensors(bool compressionPositive=false, bool splitNormalTensor=false, Real thresholdForce=NaN);
		...
		SUDODEM_PYREGISTER_CLASS_API static py::tuple fabricTensor(bool splitTensor=false, bool revertSign=false, Real thresholdForce=NaN);
		...
		SUDODEM_PYREGISTER_CLASS_API static py::list getBodyIdsContacts(Body::id_t bodyID=-1);
		...
		//! Flip cell shear without affecting interactions
		static Matrix3r flipCell(const Matrix3r& flip=Matrix3r::Zero());
