...
void Integrator::slaves_set(const pybind11::list& slaves2){
...
pybind11::list Integrator::slaves_get(){
	pybind11::list ret;
	for (auto& grp : slaves){
...
// pyRegisterClass implementation moved from Integrator.hpp
void Integrator::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Integrator");
	pybind11::class_<Integrator, TimeStepper, std::shared_ptr<Integrator>> _classObj(_module, "Integrator", "Integration Engine Interface.");
	_classObj.def(pybind11::init<>());
	_classObj.def_property("slaves",&Integrator::slaves_get,&Integrator::slaves_set,"List of lists of Engines to calculate the force acting on the particles");
	_classObj.def_readwrite("integrationsteps", &Integrator::integrationsteps, "all integrationsteps count as all succesfull substeps");
	_classObj.def_readwrite("maxVelocitySq", &Integrator::maxVelocitySq, "store square of max. velocity");
}
