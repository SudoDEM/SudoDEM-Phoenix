...
void PeriTriaxController::action()
{
	...
	if(dynCell && isnan(mass)){
		mass=0; for(const shared_ptr<Body>& b :  *scene->bodies){ if(b && b->state) mass+=b->state->mass; }
...
void PeriIsoCompressor::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("PeriIsoCompressor");
	pybind11::class_<PeriIsoCompressor, BoundaryController, std::shared_ptr<PeriIsoCompressor>> _classObj(_module, "PeriIsoCompressor", "Periodic isochoric compressor engine.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("stresses", &PeriIsoCompressor::stresses, "Stress values");
	_classObj.def_readwrite("charLen", &PeriIsoCompressor::charLen, "Characteristic length");
	_classObj.def_readwrite("maxSpan", &PeriIsoCompressor::maxSpan, "Maximum span");
	_classObj.def_readwrite("maxUnbalanced", &PeriIsoCompressor::maxUnbalanced, "Maximum unbalanced force ratio");
	_classObj.def_readwrite("globalUpdateInt", &PeriIsoCompressor::globalUpdateInt, "Global update interval");
	_classObj.def_readwrite("state", &PeriIsoCompressor::state, "Current state");
	_classObj.def_readwrite("doneHook", &PeriIsoCompressor::doneHook, "Python command to run when done");
	_classObj.def_readwrite("keepProportions", &PeriIsoCompressor::keepProportions, "Keep proportions flag");
	_classObj.def_readwrite("avgStiffness", &PeriIsoCompressor::avgStiffness, "Average stiffness");
	_classObj.def_readwrite("maxDisplPerStep", &PeriIsoCompressor::maxDisplPerStep, "Maximum displacement per step");
	_classObj.def_readwrite("sumForces", &PeriIsoCompressor::sumForces, "Sum of forces");
	_classObj.def_readwrite("sigma", &PeriIsoCompressor::sigma, "Stress vector");
	_classObj.def_readwrite("currUnbalanced", &PeriIsoCompressor::currUnbalanced, "Current unbalanced force ratio");
}

void PeriTriaxController::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("PeriTriaxController");
...
void Peri3dController::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Peri3dController");
	pybind11::class_<Peri3dController, BoundaryController, std::shared_ptr<Peri3dController>> _classObj(_module, "Peri3dController", "Experimental controller of full strain/stress tensors on periodic cell.");
	_classObj.def(pybind11::init<>());
}
