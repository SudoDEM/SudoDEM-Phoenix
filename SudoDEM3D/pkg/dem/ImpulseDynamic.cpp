...
void ImpulseDynamic::action()
{
	...
	if(scene->isPeriodic && ((prevCellSize!=scene->cell->getSize())) && /* initial value */!isnan(prevCellSize[0]) ){ cellChanged=true; maxVelocitySq=(prevCellSize-scene->cell->getSize()).squaredNorm()/pow(dt,2); }
		...
		for(const shared_ptr<BodyCallback>& cb :  callbacks){
	...
	#ifdef SUDODEM_OPENMP
		for(Real& thrMaxVSq :  threadMaxVelocitySq) { thrMaxVSq=0; }
	#endif
	...
	SUDODEM_PARALLEL_FOREACH_BODY_BEGIN(const shared_ptr<Body>& b, scene->bodies){
      ...
      if (b->shape->getClassName()=="TriElement") continue;
			...
			#ifdef SUDODEM_DEBUG
				if(isnan(f[0])||isnan(f[1])||isnan(f[2])) throw runtime_error(("ImpulseDynamic: NaN force acting on #"+std::to_string(id)+".").c_str());
				if(isnan(m[0])||isnan(m[1])||isnan(m[2])) throw runtime_error(("ImpulseDynamic: NaN torque acting on #"+std::to_string(id)+".").c_str());
				if(state->mass<=0 && ((state->blockedDOFs & State::DOF_XYZ) != State::DOF_XYZ)) throw runtime_error(("ImpulseDynamic: #"+std::to_string(id)+" has some linear accelerations enabled, but State::mass is non-positive."));
				if(state->inertia.minCoeff()<=0 && ((state->blockedDOFs & State::DOF_RXRYRZ) != State::DOF_RXRYRZ)) throw runtime_error(("ImpulseDynamic: #"+std::to_string(id)+" has some angular accelerations enabled, but State::inertia contains non-positive terms."));
			#endif
  ...
  //loop all contacts
	...
	#ifdef SUDODEM_OPENMP
		for(const Real& thrMaxVSq :  threadMaxVelocitySq) { maxVelocitySq=max(maxVelocitySq,thrMaxVSq); }
	#endif
...
void ImpulseDynamic::leapfrogTranslate(State* state, const Body::id_t& id, const Real& dt){
	...
	if ( (mask<=0) || ((mask>0) and (Body::byId(id)->maskCompatible(mask))) ) {
...
void ImpulseDynamic::leapfrogSphericalRotate(State* state, const Body::id_t& id, const Real& dt )
{
	...
	if (angle2!=0 and ( (mask<=0) || ((mask>0) and (Body::byId(id)->maskCompatible(mask))) )) {//If we have an angular velocity, we make a rotation
	...
	if(scene->forces.getMoveRotUsed() && scene->forces.getRot(id)!=Vector3r::Zero()
		and ( (mask<=0) || ((mask>0) and (Body::byId(id)->maskCompatible(mask))) )) {
...
bool ImpulseDynamic::get_densityScaling() {
	for(const shared_ptr<Engine> e :  Omega::instance().getScene()->engines) {
...
// pyRegisterClass implementation moved from ImpulseDynamic.hpp
void ImpulseDynamic::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("ImpulseDynamic");
	pybind11::class_<ImpulseDynamic, GlobalEngine, std::shared_ptr<ImpulseDynamic>> _classObj(_module, "ImpulseDynamic", "Engine integrating newtonian motion equations.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("damping", &ImpulseDynamic::damping, "damping coefficient for Cundall's non viscous damping (see [Chareyre2005]_) [-]");
	_classObj.def_readwrite("isSuperquadrics", &ImpulseDynamic::isSuperquadrics, "Enable optimation for Superquadrics");
	_classObj.def_readwrite("quiet_system_flag", &ImpulseDynamic::quiet_system_flag, "the flag for quiet system");
	_classObj.def_readwrite("gravity", &ImpulseDynamic::gravity, "Gravitational acceleration (effectively replaces GravityEngine).");
	_classObj.def_readwrite("maxVelocitySq", &ImpulseDynamic::maxVelocitySq, "store square of max. velocity, for informative purposes; computed again at every step. |yupdate|");
	_classObj.def_readwrite("exactAsphericalRot", &ImpulseDynamic::exactAsphericalRot, "Enable more exact body rotation integrator for :yref:`aspherical bodies<Body.aspherical>` *only*, using formulation from [Allen1989]_, pg. 89.");
	_classObj.def_readwrite("prevVelGrad", &ImpulseDynamic::prevVelGrad, "Store previous velocity gradient (:yref:`Cell::velGrad`) to track acceleration. |yupdate|");
	_classObj.def_readwrite("prevCellSize", &ImpulseDynamic::prevCellSize, "cell size from previous step, used to detect change and find max velocity");
	_classObj.def_readwrite("warnNoForceReset", &ImpulseDynamic::warnNoForceReset, "Warn when forces were not resetted in this step by :yref:`ForceResetter`; this mostly points to :yref:`ForceResetter` being forgotten incidentally and should be disabled only with a good reason.");
	_classObj.def_readwrite("nonviscDampIx", &ImpulseDynamic::nonviscDampIx, "Index of the energy dissipated using the non-viscous damping (:yref:`damping<ImpulseDynamic.damping>`).");
	_classObj.def_readwrite("kinSplit", &ImpulseDynamic::kinSplit, "Whether to separately track translational and rotational kinetic energy.");
	_classObj.def_readwrite("kinEnergyIx", &ImpulseDynamic::kinEnergyIx, "Index for kinetic energy in scene->energies.");
	_classObj.def_readwrite("kinEnergyTransIx", &ImpulseDynamic::kinEnergyTransIx, "Index for translational kinetic energy in scene->energies.");
	_classObj.def_readwrite("kinEnergyRotIx", &ImpulseDynamic::kinEnergyRotIx, "Index for rotational kinetic energy in scene->energies.");
	_classObj.def_readwrite("mask", &ImpulseDynamic::mask, "If mask defined and the bitwise AND between mask and body`s groupMask gives 0, the body will not move/rotate. Velocities and accelerations will be calculated not paying attention to this parameter.");
	_classObj.def_property("densityScaling", &ImpulseDynamic::get_densityScaling, &ImpulseDynamic::set_densityScaling, "if True, then density scaling will be applied.");
}
