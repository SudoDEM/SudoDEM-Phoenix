...
void Disp2DPropLoadEngine::action()
{
	...
	{
		stopMovement();
		string fileName=Key + "DR"+std::to_string(nbre_iter)+"ItAtV_"+std::to_string(v)+"done.xml";
...
void Disp2DPropLoadEngine::saveData()
{
	...
	ofile << std::to_string(theta) << " "<< std::to_string(dTau) << " " << std::to_string(dSigN) << " "
		<< std::to_string(dgamma)<<" " << std::to_string(du) << " " << std::to_string(Tau0) << " "
		<< std::to_string(SigN0) << " " << std::to_string(d2W) << " "
		<< std::to_string(coordSs0) << " " << std::to_string(coordTot0) << " "
		<< std::to_string(coordSs) << " " << std::to_string(coordTot) <<endl;
}

// pyRegisterClass implementation moved from Disp2DPropLoadEngine.hpp
void Disp2DPropLoadEngine::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Disp2DPropLoadEngine");
	pybind11::class_<Disp2DPropLoadEngine, BoundaryController, std::shared_ptr<Disp2DPropLoadEngine>> _classObj(_module, "Disp2DPropLoadEngine", "Disturbs a simple shear sample in a given displacement direction.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("id_topbox", &Disp2DPropLoadEngine::id_topbox, "the id of the upper wall");
	_classObj.def_readwrite("id_boxbas", &Disp2DPropLoadEngine::id_boxbas, "the id of the lower wall");
	_classObj.def_readwrite("id_boxleft", &Disp2DPropLoadEngine::id_boxleft, "the id of the left wall");
	_classObj.def_readwrite("id_boxright", &Disp2DPropLoadEngine::id_boxright, "the id of the right wall");
	_classObj.def_readwrite("id_boxfront", &Disp2DPropLoadEngine::id_boxfront, "the id of the wall in front of the sample");
	_classObj.def_readwrite("id_boxback", &Disp2DPropLoadEngine::id_boxback, "the id of the wall at the back of the sample");
	_classObj.def_readwrite("theta", &Disp2DPropLoadEngine::theta, "the angle, in a (gamma,h=-u) plane from the gamma - axis to the perturbation vector (trigo wise) [degrees]");
	_classObj.def_readwrite("v", &Disp2DPropLoadEngine::v, "the speed at which the perturbation is imposed.");
	_classObj.def_readwrite("nbre_iter", &Disp2DPropLoadEngine::nbre_iter, "the number of iterations of loading to perform");
	_classObj.def_readwrite("Key", &Disp2DPropLoadEngine::Key, "string to add at the names of the saved files, and of the output file filled by *saveData*");
	_classObj.def_readwrite("LOG", &Disp2DPropLoadEngine::LOG, "boolean controling the output of messages on the screen");
}
