...
//#define USE_MARGIN
	...
	SUDODEM_PLUGIN(Gl1_GJKParticle, Gl1_GJKParticleGeom);
#endif

REGISTER_CLASS_INDEX_CPP(GJKParticle,Shape)
REGISTER_CLASS_INDEX_CPP(GJKParticleGeom,IGeom)
REGISTER_CLASS_INDEX_CPP(GJKParticleMat,Material)
REGISTER_CLASS_INDEX_CPP(GJKParticlePhys,FrictPhys)

// Destructor definitions - required for typeinfo generation
GJKParticleLaw::~GJKParticleLaw() {}
...
void Bo1_GJKParticle_Aabb::go(const shared_ptr<Shape>& ig, shared_ptr<Bound>& bv, const Se3r& se3, const Body*){
  //if(!ig){cout<<"the pointer of ig is null at aabb_go."<<endl;}
	...
	//cout<<"rot="<<rot<<endl;
	Vector3r thismin(- t->supportH(-rot.row(0)) ,
               - t->supportH(-rot.row(1)),
		       - t->supportH(-rot.row(2)) );
	Vector3r thismax( t->supportH( rot.row(0)) ,
               + t->supportH( rot.row(1)),
               + t->supportH( rot.row(2)) );

	//cout<<"min max1 "<<min<<max<<endl;
	thismin += se3.position;
	thismax += se3.position;
	//min += se3.position-t->getCentroid();
	//max += se3.position-t->getCentroid();
	//cout<<"min max"<<min<<max<<endl;
	//Vector3r max = t->getBBox().getMax();
	double margin = t->getMargin();
	aabb->min=Vector3r(thismin[0]-margin, thismin[1]-margin, thismin[2]-margin);//se3.position+mincoords;
	aabb->max=Vector3r(thismax[0]+margin, thismax[1]+margin, thismax[2]+margin);//se3.position+maxcoords;
}
...
// pyRegisterClass implementation moved from GJKParticle.hpp
void GJKParticle::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("GJKParticle");
	pybind11::class_<GJKParticle, Shape, std::shared_ptr<GJKParticle>> _classObj(_module, "GJKParticle", "GJK Particle shape");
	_classObj.def(pybind11::init<>());
	_classObj.def("Initial",&GJKParticle::Initial,"Initialization");
	_classObj.def("getVolume",&GJKParticle::getVolume,"return particle's volume");
	_classObj.def("getInertia",&GJKParticle::getInertia,"return particle's inertia tensor");
	_classObj.def("getOri",&GJKParticle::getOrientation,"return particle's orientation");
	_classObj.def("getCentroid",&GJKParticle::getPosition,"return particle's centroid");
}

// pyRegisterClass implementation moved from GJKParticle.hpp
void GJKParticleGeom::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("GJKParticleGeom");
	pybind11::class_<GJKParticleGeom, IGeom, std::shared_ptr<GJKParticleGeom>> _classObj(_module, "GJKParticleGeom", "Geometry of interaction between 2 GJKParticles");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("PenetrationDepth", &GJKParticleGeom::PenetrationDepth, "PenetrationDepth");
}

// pyRegisterClass implementation moved from GJKParticle.hpp
void Bo1_GJKParticle_Aabb::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Bo1_GJKParticle_Aabb");
	pybind11::class_<Bo1_GJKParticle_Aabb, BoundFunctor, std::shared_ptr<Bo1_GJKParticle_Aabb>> _classObj(_module, "Bo1_GJKParticle_Aabb", "Create Aabb from GJKParticle");
	_classObj.def(pybind11::init<>());
}

// pyRegisterClass implementation moved from GJKParticle.hpp
void GJKParticleMat::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("GJKParticleMat");
	pybind11::class_<GJKParticleMat, Material, std::shared_ptr<GJKParticleMat>> _classObj(_module, "GJKParticleMat", "Elastic material with Coulomb friction");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("Kn", &GJKParticleMat::Kn, "Normal stiffness (N/m)");
	_classObj.def_readwrite("Ks", &GJKParticleMat::Ks, "Shear stiffness (N/m)");
	_classObj.def_readwrite("frictionAngle", &GJKParticleMat::frictionAngle, "Contact friction angle");
	_classObj.def_readwrite("betan", &GJKParticleMat::betan, "Normal Damping Ratio");
	_classObj.def_readwrite("betas", &GJKParticleMat::betas, "Shear Damping Ratio");
	_classObj.def_readwrite("IsSplitable", &GJKParticleMat::IsSplitable, "To be splitted");
	_classObj.def_readwrite("strength", &GJKParticleMat::strength, "Stress at which polyhedra breaks");
}
