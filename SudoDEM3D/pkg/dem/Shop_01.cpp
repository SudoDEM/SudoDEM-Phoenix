...
Vector3r Shop::totalForceInVolume(Real& avgIsoStiffness, Scene* _rb){
	...
	for(const shared_ptr<Interaction>&I :  *rb->interactions){
...
Real Shop::unbalancedForce(bool useMaxForce, Scene* _rb){
	...
	for(shared_ptr<Engine>& e :  rb->engines){ newton=SUDODEM_PTR_DYN_CAST<NewtonIntegrator>(e); if(newton) {gravity=newton->gravity; break;} }
	// get maximum force on a body and sum of all forces (for averaging)
	Real sumF=0,maxF=0,currF; int nb=0;int id_max;
	for(const shared_ptr<Body>& b :  *rb->bodies){
...
shared_ptr<Body> Shop::sphere(Vector3r center, Real radius, shared_ptr<Material> mat){
	...
	body->material=mat ? mat : std::static_pointer_cast<Material>(defaultGranularMat());
...
Real Shop::getSpheresVolume(const shared_ptr<Scene>& _scene, int mask){
	...
	for(shared_ptr<Body> b :  *scene->bodies){
		...
		if((!s) || ((mask>0) && ((b->groupMask & mask)==0))) continue;
...
Real Shop::getVoxelPorosity(const shared_ptr<Scene>& _scene, int _resolution, Vector3r _start,Vector3r _end){
	...
	for(shared_ptr<Body> bi :  *scene->bodies){
...
vector<std::tuple<Vector3r,Real,int> > Shop::loadSpheresFromFile(const string& fname, Vector3r& minXYZ, Vector3r& maxXYZ, Vector3r* cellSize){
	if(!std::filesystem::exists(fname)) {
		throw std::invalid_argument(string("File with spheres `")+fname+"' doesn't exist.");
	}
	vector<std::tuple<Vector3r,Real,int> > spheres;
	...
	while(std::getline(sphereFile, line, '\n')){
		lineNo++;
		// Tokenize using std::istringstream
		...
		if(tokens[0]=="##PERIODIC::"){
			if(tokens.size()!=4) throw std::invalid_argument(("Spheres file "+fname+":"+std::to_string(lineNo)+" contains ##PERIODIC::, but the line is malformed.").c_str());
			if(cellSize){ *cellSize=Vector3r(std::stod(tokens[1]),std::stod(tokens[2]),std::stod(tokens[3])); }
			continue;
		}
		if(tokens.size()!=5 && tokens.size()!=4) throw std::invalid_argument(("Line "+std::to_string(lineNo)+" in the spheres file "+fname+" has "+std::to_string(tokens.size())+" columns (must be 4 or 5).").c_str());
		...
		if(tokens.size()==5)clumpId=std::stoi(tokens[4]);
		spheres.push_back(std::tuple<Vector3r,Real,int>(C,r,clumpId));
	}
	return spheres;
}
