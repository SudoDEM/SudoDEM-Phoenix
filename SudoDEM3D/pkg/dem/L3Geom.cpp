...
REGISTER_CLASS_INDEX_CPP(L3Geom,GenericSpheresContact)
REGISTER_CLASS_INDEX_CPP(L6Geom,L3Geom)
...
void Gl1_L3Geom::draw(const shared_ptr<IGeom>& ig, bool isL6Geom, const Real& phiScale){
	...
	if(axesWd>0){
		...
		for(int i=0; i<3; i++){
			...
			if(axesLabels) GLUtils::GLDrawText(string(i==0?"x":(i==1?"y":"z")),Vector2r(pt[0],pt[1]),color);
		}
	}
	if(uPhiWd>0){
		glLineWidth(uPhiWd);
...
// pyRegisterClass implementations moved from L3Geom.hpp
void L3Geom::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("L3Geom");
	pybind11::class_<L3Geom, GenericSpheresContact, std::shared_ptr<L3Geom>> _classObj(_module, "L3Geom", "Geometry of contact given in local coordinates with 3 degress of freedom");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("u", &L3Geom::u, "Displacement components, in local coordinates. |yupdate|");
	_classObj.def_readwrite("u0", &L3Geom::u0, "Zero displacement value");
	_classObj.def_readwrite("trsf", &L3Geom::trsf, "Transformation (rotation) from global to local coordinates");
	_classObj.def_readwrite("F", &L3Geom::F, "Applied force in local coordinates [debugging only]");
}

void L6Geom::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("L6Geom");
	pybind11::class_<L6Geom, L3Geom, std::shared_ptr<L6Geom>> _classObj(_module, "L6Geom", "Geometric of contact in local coordinates with 6 degrees of freedom");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("phi", &L6Geom::phi, "Rotation components, in local coordinates. |yupdate|");
	_classObj.def_readwrite("phi0", &L6Geom::phi0, "Zero rotation, should be always subtracted from phi to get the value");
}

void Ig2_Sphere_Sphere_L3Geom::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Ig2_Sphere_Sphere_L3Geom");
	pybind11::class_<Ig2_Sphere_Sphere_L3Geom, IGeomFunctor, std::shared_ptr<Ig2_Sphere_Sphere_L3Geom>> _classObj(_module, "Ig2_Sphere_Sphere_L3Geom", "Incrementally compute L3Geom for contact of 2 spheres");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("noRatch", &Ig2_Sphere_Sphere_L3Geom::noRatch, "See avoidGranularRatcheting");
	_classObj.def_readwrite("distFactor", &Ig2_Sphere_Sphere_L3Geom::distFactor, "Create interaction if spheres are not futher than distFactor*(r1+r2)");
	_classObj.def_readwrite("trsfRenorm", &Ig2_Sphere_Sphere_L3Geom::trsfRenorm, "How often to renormalize trsf");
	_classObj.def_readwrite("approxMask", &Ig2_Sphere_Sphere_L3Geom::approxMask, "Selectively enable geometrical approximations (bitmask)");
}

void Ig2_Wall_Sphere_L3Geom::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Ig2_Wall_Sphere_L3Geom");
	pybind11::class_<Ig2_Wall_Sphere_L3Geom, Ig2_Sphere_Sphere_L3Geom, std::shared_ptr<Ig2_Wall_Sphere_L3Geom>> _classObj(_module, "Ig2_Wall_Sphere_L3Geom", "Incrementally compute L3Geom for contact between Wall and Sphere");
	_classObj.def(pybind11::init<>());
}

void Ig2_Facet_Sphere_L3Geom::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Ig2_Facet_Sphere_L3Geom");
	pybind11::class_<Ig2_Facet_Sphere_L3Geom, Ig2_Sphere_Sphere_L3Geom, std::shared_ptr<Ig2_Facet_Sphere_L3Geom>> _classObj(_module, "Ig2_Facet_Sphere_L3Geom", "Incrementally compute L3Geom for contact between Facet and Sphere");
	_classObj.def(pybind11::init<>());
}

void Ig2_Sphere_Sphere_L6Geom::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Ig2_Sphere_Sphere_L6Geom");
	pybind11::class_<Ig2_Sphere_Sphere_L6Geom, Ig2_Sphere_Sphere_L3Geom, std::shared_ptr<Ig2_Sphere_Sphere_L6Geom>> _classObj(_module, "Ig2_Sphere_Sphere_L6Geom", "Incrementally compute L6Geom for contact of 2 spheres");
	_classObj.def(pybind11::init<>());
}

void Law2_L3Geom_FrictPhys_ElPerfPl::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Law2_L3Geom_FrictPhys_ElPerfPl");
	pybind11::class_<Law2_L3Geom_FrictPhys_ElPerfPl, LawFunctor, std::shared_ptr<Law2_L3Geom_FrictPhys_ElPerfPl>> _classObj(_module, "Law2_L3Geom_FrictPhys_ElPerfPl", "Basic law for testing L3Geom");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("noBreak", &Law2_L3Geom_FrictPhys_ElPerfPl::noBreak, "Do not break contacts when particles separate");
	_classObj.def_readwrite("noSlip", &Law2_L3Geom_FrictPhys_ElPerfPl::noSlip, "No plastic slipping");
}

void Law2_L6Geom_FrictPhys_Linear::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Law2_L6Geom_FrictPhys_Linear");
	pybind11::class_<Law2_L6Geom_FrictPhys_Linear, Law2_L3Geom_FrictPhys_ElPerfPl, std::shared_ptr<Law2_L6Geom_FrictPhys_Linear>> _classObj(_module, "Law2_L6Geom_FrictPhys_Linear", "Basic law for testing L6Geom -- linear in both normal and shear sense");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("charLen", &Law2_L6Geom_FrictPhys_Linear::charLen, "Characteristic length with the meaning of the stiffness ratios");
}
