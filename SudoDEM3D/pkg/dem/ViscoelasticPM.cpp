...
REGISTER_CLASS_INDEX_CPP(ViscElMat,FrictMat)
REGISTER_CLASS_INDEX_CPP(ViscElPhys,FrictPhys)
...
bool Law2_ScGeom_ViscElPhys_Basic::go(shared_ptr<IGeom>& _geom, shared_ptr<IPhys>& _phys, Interaction* I) {
	...
	if (computeForceTorqueViscEl(_geom, _phys, I, force, torque1, torque2) && (I->isActive)) {
...
void Ip2_ViscElMat_ViscElMat_ViscElPhys::Calculate_ViscElMat_ViscElMat_ViscElPhys(const shared_ptr<Material>& b1, const shared_ptr<Material>& b2, const shared_ptr<Interaction>& interaction, shared_ptr<ViscElPhys> phys) {
	...
	if ((isfinite(mat1->kn) &&  ! (isfinite(mat2->kn))) ||
			(isfinite(mat2->kn) &&  ! (isfinite(mat1->kn))) ||
			(isfinite(mat1->ks) &&  ! (isfinite(mat2->ks))) ||
			(isfinite(mat2->ks) &&  ! (isfinite(mat1->ks))) ||
			(isfinite(mat1->cn) &&  ! (isfinite(mat2->cn))) ||
			(isfinite(mat2->cn) &&  ! (isfinite(mat1->cn))) ||
			(isfinite(mat1->cs) &&  ! (isfinite(mat2->cs))) ||
			(isfinite(mat2->cs) &&  ! (isfinite(mat1->cs))) ||
			(isfinite(mat1->tc) &&  ! (isfinite(mat2->tc))) ||
			(isfinite(mat2->tc) &&  ! (isfinite(mat1->tc))) ||
			(isfinite(mat1->en) &&  ! (isfinite(mat2->en))) ||
			(isfinite(mat2->en) &&  ! (isfinite(mat1->en))) ||
			(isfinite(mat1->et) &&  ! (isfinite(mat2->et))) ||
			(isfinite(mat2->et) &&  ! (isfinite(mat1->et)))) {
	...
	if (((isfinite(mat1->tc)) && (isfinite(mat1->en)) && (isfinite(mat1->et))) || ((tc) && (en) && (et))) {
    ...
    // http://www.mail-archive.com/sudodem-users@lists.launchpad.net/msg08741.html
	...
	} else if ((isfinite(mat1->kn)) && (isfinite(mat1->ks)) && (isfinite(mat1->cn)) && (isfinite(mat1->cs))) {
		...
		if ((isfinite(mat1->cn)) && (isfinite(mat1->cs))) {
			cn1 = mat1->cn;
			cn2 = mat2->cn;
			cs1 = mat1->cs;
			cs2 = mat2->cs;
...
void ViscElMat::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("ViscElMat");
	pybind11::class_<ViscElMat, FrictMat, std::shared_ptr<ViscElMat>> _classObj(_module, "ViscElMat", "Material for simple viscoelastic model of contact.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("tc", &ViscElMat::tc, "Contact time");
	_classObj.def_readwrite("en", &ViscElMat::en, "Restitution coefficient in normal direction");
	_classObj.def_readwrite("et", &ViscElMat::et, "Restitution coefficient in tangential direction");
	_classObj.def_readwrite("kn", &ViscElMat::kn, "Normal elastic stiffness");
	_classObj.def_readwrite("cn", &ViscElMat::cn, "Normal viscous constant");
	_classObj.def_readwrite("ks", &ViscElMat::ks, "Shear elastic stiffness");
	_classObj.def_readwrite("cs", &ViscElMat::cs, "Shear viscous constant");
	_classObj.def_readwrite("mR", &ViscElMat::mR, "Rolling resistance");
	_classObj.def_readwrite("mRtype", &ViscElMat::mRtype, "Rolling resistance type");
}

void ViscElPhys::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("ViscElPhys");
	pybind11::class_<ViscElPhys, FrictPhys, std::shared_ptr<ViscElPhys>> _classObj(_module, "ViscElPhys", "Physics for viscoelastic contact.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("cn", &ViscElPhys::cn, "Normal viscous constant");
	_classObj.def_readwrite("cs", &ViscElPhys::cs, "Shear viscous constant");
	_classObj.def_readwrite("mR", &ViscElPhys::mR, "Rolling resistance");
	_classObj.def_readwrite("mRtype", &ViscElPhys::mRtype, "Rolling resistance type");
}

void Ip2_ViscElMat_ViscElMat_ViscElPhys::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Ip2_ViscElMat_ViscElMat_ViscElPhys");
	pybind11::class_<Ip2_ViscElMat_ViscElMat_ViscElPhys, IPhysFunctor, std::shared_ptr<Ip2_ViscElMat_ViscElMat_ViscElPhys>> _classObj(_module, "Ip2_ViscElMat_ViscElMat_ViscElPhys", "Convert 2 instances of ViscElMat to ViscElPhys.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("tc", &Ip2_ViscElMat_ViscElMat_ViscElPhys::tc, "Instance of MatchMaker determining contact time");
	_classObj.def_readwrite("en", &Ip2_ViscElMat_ViscElMat_ViscElPhys::en, "Instance of MatchMaker determining restitution coefficient in normal direction");
	_classObj.def_readwrite("et", &Ip2_ViscElMat_ViscElMat_ViscElPhys::et, "Instance of MatchMaker determining restitution coefficient in tangential direction");
}

void Law2_ScGeom_ViscElPhys_Basic::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Law2_ScGeom_ViscElPhys_Basic");
	pybind11::class_<Law2_ScGeom_ViscElPhys_Basic, LawFunctor, std::shared_ptr<Law2_ScGeom_ViscElPhys_Basic>> _classObj(_module, "Law2_ScGeom_ViscElPhys_Basic", "Linear viscoelastic model.");
	_classObj.def(pybind11::init<>());
}
