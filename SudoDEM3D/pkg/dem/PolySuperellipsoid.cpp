...
REGISTER_CLASS_INDEX_CPP(PolySuperellipsoid,Shape)
REGISTER_CLASS_INDEX_CPP(PolySuperellipsoidGeom,IGeom)
REGISTER_CLASS_INDEX_CPP(PolySuperellipsoidMat,Material)
REGISTER_CLASS_INDEX_CPP(PolySuperellipsoidPhys,FrictPhys)
...
/* Plotting */
	...
	// Note: wire and slices are inline static members defined in the header	int Gl1_PolySuperellipsoid::pre_slices = -1;
...
void Gl1_PolySuperellipsoid::initSolidGlList(PolySuperellipsoid* t)
{
	...
	for(a=0.0,i=0;i<h;i++,a+=hStep)
	  ...
	  {
		vertices.push_back(t->getSurfaceMC(Vector2r(b,a-Mathr::HALF_PI)));
	  }
...
// Static member definitions for Gl1_PolySuperellipsoid
bool Gl1_PolySuperellipsoid::wire = true;
int Gl1_PolySuperellipsoid::slices = 5;
