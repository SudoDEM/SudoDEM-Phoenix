...
void NewtonIntegrator::action()
{
	...
	SUDODEM_PARALLEL_FOREACH_BODY_BEGIN(const shared_ptr<Body>& b, scene->bodies){
			...
			#ifdef SUDODEM_DEBUG
				if(isnan(f[0])||isnan(f[1])||isnan(f[2])) throw runtime_error(("NewtonIntegrator: NaN force acting on #"+std::to_string(id)+".").c_str());
				if(isnan(m[0])||isnan(m[1])||isnan(m[2])) throw runtime_error(("NewtonIntegrator: NaN torque acting on #"+std::to_string(id)+".").c_str());
				if(state->mass<=0 && ((state->blockedDOFs & State::DOF_XYZ) != State::DOF_XYZ)) throw runtime_error(("NewtonIntegrator: #"+std::to_string(id)+" has some linear accelerations enabled, but State::mass is non-positive."));
				if(state->inertia.minCoeff()<=0 && ((state->blockedDOFs & State::DOF_RXRYRZ) != State::DOF_RXRYRZ)) throw runtime_error(("NewtonIntegrator: #"+std::to_string(id)+" has some angular accelerations enabled, but State::inertia contains non-positive terms."));
			#endif
...
void NewtonIntegrator::leapfrogSphericalRotate(State* state, const Body::id_t& id, const Real& dt )
{
	...
	if (angle2!=0 && ( (mask<=0) || ((mask>0) && (Body::byId(id)->maskCompatible(mask))) )) {//If we have an angular velocity, we make a rotation
	...
	if(scene->forces.getMoveRotUsed() && scene->forces.getRot(id)!=Vector3r::Zero()
		&& ( (mask<=0) || ((mask>0) && (Body::byId(id)->maskCompatible(mask))) )) {
...
void NewtonIntegrator::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("NewtonIntegrator");
	...
	_classObj.def(pybind11::init([](Real damping, Vector3r gravity, std::string label, int isSuperquadrics) {
		auto n = std::make_shared<NewtonIntegrator>();
		n->damping = damping;
		n->gravity = gravity;
		n->label = label;
		n->isSuperquadrics = isSuperquadrics;
		return n;
	}), py::arg("damping") = 0.0, py::arg("gravity") = Vector3r::Zero(), py::arg("label") = "", py::arg("isSuperquadrics") = 0);
	...
	_classObj.def_readwrite("isSuperquadrics", &NewtonIntegrator::isSuperquadrics, "Enable optimation for Superquadrics:1 for superellipsoid, 2 for poly-superellipsoid, 4 for GJKParticle");
	...
	_classObj.def_readwrite("maxVelocitySq", &NewtonIntegrator::maxVelocitySq, "store square of max. velocity, for informative purposes; computed again at every step.");
	_classObj.def_readwrite("exactAsphericalRot", &NewtonIntegrator::exactAsphericalRot, "Enable more exact body rotation integrator for aspherical bodies only, using formulation from [Allen1989]_, pg. 89.");
	_classObj.def_readwrite("prevVelGrad", &NewtonIntegrator::prevVelGrad, "Store previous velocity gradient to track acceleration.");
	...
	_classObj.def_readwrite("warnNoForceReset", &NewtonIntegrator::warnNoForceReset, "Warn when forces were not resetted in this step by ForceResetter; this mostly points to ForceResetter being forgotten incidentally and should be disabled only with a good reason.");
	_classObj.def_readwrite("nonviscDampIx", &NewtonIntegrator::nonviscDampIx, "Index of the energy dissipated using the non-viscous damping.");
	...
	_classObj.def_property("densityScaling", &NewtonIntegrator::get_densityScaling, &NewtonIntegrator::set_densityScaling, "if True, then density scaling will be applied.");
}
