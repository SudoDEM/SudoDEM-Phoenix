...
public:
	...
	void scale(Real scale){ Vector3r mid=midPt(); cellSize*=scale; for(Sph& s : pack) {s.c=scale*(s.c-mid)+mid; s.r*=abs(scale); } }
	#if 0
	...
	py::tuple getitem(size_t idx){ if(idx>=pack.size()) throw runtime_error("Index "+std::to_string(idx)+" out of range 0.."+std::to_string(pack.size()-1)); return pack[idx].asTuple(); }
	struct _iterator{
		const SpherePack& sPack; size_t pos;
		_iterator(const SpherePack& _sPack): sPack(_sPack), pos(0){}
		_iterator iter(){ return *this;}
		py::tuple next(){
			if(pos==sPack.pack.size()){ throw pybind11::stop_iteration(); }
