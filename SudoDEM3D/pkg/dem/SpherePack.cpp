...
void SpherePack::fromList(const py::list& l){
	...
	for(size_t i=0; i<len; i++){
		try {
			py::tuple t = l[i].cast<py::tuple>();
			Vector3r vec = t[0].cast<Vector3r>();
			Real radius = t[1].cast<Real>();
			int clumpId = (py::len(t)>2) ? t[2].cast<int>() : -1;
			pack.push_back(Sph(vec, radius, clumpId));
...
void SpherePack::fromLists(const vector<Vector3r>& centers, const vector<Real>& radii){
	pack.clear();
	if(centers.size()!=radii.size()) throw std::invalid_argument(("The same number of centers and radii must be given (is "+std::to_string(centers.size())+", "+std::to_string(radii.size())+")").c_str());
...
py::list SpherePack::toList() const {
	py::list ret;
	for(const Sph& s :  pack) ret.append(s.asTuple());
	return ret;
};

void SpherePack::fromFile(string file) {
	typedef std::tuple<Vector3r,Real,int> tupleVector3rRealInt;
	vector<tupleVector3rRealInt> ss;
	Vector3r mn,mx;
	ss=Shop::loadSpheresFromFile(file,mn,mx,&cellSize);
	pack.clear();
	for(const tupleVector3rRealInt& s :  ss) pack.push_back(Sph(std::get<0>(s),std::get<1>(s),std::get<2>(s)));
}
...
long SpherePack::makeCloud(Vector3r mn, Vector3r mx, Real rMean, Real rRelFuzz, int num, bool periodic, Real porosity, const vector<Real>& psdSizes, const vector<Real>& psdCumm, bool distributeMass, int seed, Matrix3r hSize){
	isPeriodic = periodic;
	static std::mt19937 randGen(seed!=0?seed:(int)TimingInfo::getNow(/* get the number even if timing is disabled globally */ true));
	...
	if(psdSizes.size()>0){
		...
		if(psdSizes.size()!=psdCumm.size()) throw invalid_argument(("SpherePack.makeCloud: psdSizes and psdCumm must have same dimensions ("+std::to_string(psdSizes.size())+"!="+std::to_string(psdCumm.size())).c_str());
		...
		for(size_t i=0; i<psdSizes.size(); i++) {
			...
			LOG_DEBUG(i<<". "<<psdRadii[i]<<", cdf="<<psdCumm[i]<<", cdf2="<<(distributeMass?std::to_string(psdCumm2[i]):string("--")));
			// check monotonicity
...
py::tuple SpherePack::psd(int bins, bool mass) const {
	...
	for(const Sph& s :  pack){ maxD=max(2*s.r,maxD); minD=min(2*s.r,minD); vol+=pow(s.r,3); }
...
long SpherePack::particleSD(Vector3r mn, Vector3r mx, Real rMean, bool periodic, string name, int numSph, const vector<Real>& radii, const vector<Real>& passing, bool passingIsNotPercentageButCount, int seed){
	...
	if(!passingIsNotPercentageButCount){
		...
		for(Real p :  passing) numbers.push_back(p);
	}

	static std::mt19937 randGen(seed!=0?seed:(int)TimingInfo::getNow(true));
...
long SpherePack::makeClumpCloud(const Vector3r& mn, const Vector3r& mx, const vector<shared_ptr<SpherePack> >& _clumps, bool periodic, int num, int seed){
	...
	for(const shared_ptr<SpherePack>& c :  _clumps){
		...
		clumps.push_back(c2);
		Real r=0;
		for(const Sph& s :  c2.pack) r=max(r,s.c.norm()+s.r);
		...
		for(const Sph& s :  c2.pack) maxR=max(maxR,s.r); // keep track of maximum sphere radius
	}
	...
	while(nGen<num || num<0){
		...
		while(true){ // check for tries at the end
			...
			if(!periodic){
				...
				if((pos+rad*Vector3r::Ones()).cwiseMax(mx)!=mx || (pos-rad*Vector3r::Ones()).cwiseMin(mn)!=mn){ for(const Sph& s :  C.pack) if((s.c+s.r*Vector3r::Ones()).cwiseMax(mx)!=mx || (s.c-s.r*Vector3r::Ones()).cwiseMin(mn)!=mn) goto overlap; }
				...
				for(const ClumpInfo& cInfo :  clumpInfos){
					...
					if((pos-cInfo.center).squaredNorm()<pow(rad+cInfo.rad,2)){ for(const Sph& s :  C.pack) if(pow(s.r+cInfo.rad,2)>(s.c-cInfo.center).squaredNorm()){ detailedCheck=true; break; }}
					// check sphere-by-sphere, since bounding spheres did overlap
					if(detailedCheck){ for(const Sph& s :  C.pack) for(int id=cInfo.minId; id<=cInfo.maxId; id++) if((s.c-pack[id].c).squaredNorm()<pow(s.r+pack[id].r,2)) goto overlap;}
				}
			} else {
				for(const ClumpInfo& cInfo :  clumpInfos){
					...
					if(periPtDistSq(pos,cInfo.center)<pow(rad+cInfo.rad,2)){
						...
						for(const Sph& s :  C.pack) if(pow(s.r+cInfo.rad,2)>periPtDistSq(s.c,cInfo.center)){ detailedCheck=true; break; }
						// check sphere-by-sphere
						if(detailedCheck){ for(const Sph& s :  C.pack) for(int id=cInfo.minId; id<=cInfo.maxId; id++) if(periPtDistSq(s.c,pack[id].c)<pow(s.r+pack[id].r,2)) goto overlap; }
					}
				}
			}
...
py::tuple SpherePack::getClumps() const{
	...
	for(const intListPair& c :  clumps) clumpList.append(c.second);
