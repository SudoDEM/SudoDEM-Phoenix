...
void FacetTopologyAnalyzer::action(){
	...
	int maxVertexId=0;
	for(shared_ptr<VertexData>& v :  vv){
		if(v->vertexId<0){
			assert(v->isLowestIndex);
			v->vertexId=maxVertexId++;
		}
		for(shared_ptr<VertexData>& vNext :  v->nextIdentical){
...
// pyRegisterClass implementation moved from FacetTopologyAnalyzer.hpp
void FacetTopologyAnalyzer::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("FacetTopologyAnalyzer");
	pybind11::class_<FacetTopologyAnalyzer, GlobalEngine, std::shared_ptr<FacetTopologyAnalyzer>> _classObj(_module, "FacetTopologyAnalyzer", "Initializer for filling adjacency geometry data for facets.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("projectionAxis", &FacetTopologyAnalyzer::projectionAxis, "Axis along which to do the initial vertex sort");
	_classObj.def_readwrite("relTolerance", &FacetTopologyAnalyzer::relTolerance, "maximum distance of 'identical' vertices, relative to minimum facet size");
	_classObj.def_readwrite("commonEdgesFound", &FacetTopologyAnalyzer::commonEdgesFound, "how many common edges were identified during last run. |yupdate|");
	_classObj.def_readwrite("commonVerticesFound", &FacetTopologyAnalyzer::commonVerticesFound, "how many common vertices were identified during last run. |yupdate|");
}
