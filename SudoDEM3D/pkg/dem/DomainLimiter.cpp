...
void DomainLimiter::action(){
	...
	for(const shared_ptr<Body>& b :  *scene->bodies){
		if((!b) || ((mask>0) && ((b->groupMask & mask)==0))) continue;
...
void LawTester::action(){
	...
	string strIds("##"+std::to_string(ids[0])+"+"+std::to_string(ids[1]));
...
void GlExtra_LawTester::render(){
	...
	if(!tester){ for(shared_ptr<Engine> e :  scene->engines){ tester=SUDODEM_PTR_DYN_CAST<LawTester>(e); if(tester) break; } }
	...
	for(int i=0; i<3; i++){
		...
		Vector2r pt2d(pt.x(), pt.y());
		GLUtils::GLDrawText(string(i==0?"x":(i==1?"y":"z")), pt2d, color);
	}
	...
	Real displMax=0;
	for(const Vector6r& v :  VV) displMax=max(v.head<3>().squaredNorm(),displMax);
...
// LawTester implementation
Vector3r LawTester::get_ptOurs() {
	// Return first 3 components of uTest (tester displacement)
	return uTest.head<3>();
}

Vector3r LawTester::get_ptGeom() {
	// Return first 3 components of uGeom (displacement at contact point)
	return uGeom.head<3>();
}

Vector3r LawTester::get_rotOurs() {
	// Return last 3 components of uTest (tester rotation)
	return uTest.tail<3>();
}

Vector3r LawTester::get_rotGeom() {
	// Return last 3 components of uGeom (rotation at contact point)
	return uGeom.tail<3>();
}

// pyRegisterClass implementation moved from DomainLimiter.hpp
void DomainLimiter::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("DomainLimiter");
	pybind11::class_<DomainLimiter, PeriodicEngine, std::shared_ptr<DomainLimiter>> _classObj(_module, "DomainLimiter", "Delete particles that are out of axis-aligned box given by lo and hi.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("lo", &DomainLimiter::lo, "Lower corner of the domain.");
	_classObj.def_readwrite("hi", &DomainLimiter::hi, "Upper corner of the domain.");
	_classObj.def_readonly("nDeleted", &DomainLimiter::nDeleted, "Cummulative number of particles deleted.");
	_classObj.def_readwrite("mDeleted", &DomainLimiter::mDeleted, "Mass of deleted particles.");
	_classObj.def_readwrite("vDeleted", &DomainLimiter::vDeleted, "Volume of deleted particles.");
	_classObj.def_readwrite("mask", &DomainLimiter::mask, "If mask is defined, only particles with corresponding groupMask will be deleted.");
}

// pyRegisterClass implementation moved from DomainLimiter.hpp
void LawTester::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("LawTester");
	pybind11::class_<LawTester, PartialEngine, std::shared_ptr<LawTester>> _classObj(_module, "LawTester", "Prescribe and apply deformations of an interaction in terms of local normal and shear displacements and rotations.");
	_classObj.def(pybind11::init<>());
	_classObj.def_property_readonly("ptOurs", &LawTester::get_ptOurs, "first 3 components of uTest |ydeprecated|");
	_classObj.def_property_readonly("ptGeom", &LawTester::get_ptGeom, "first 3 components of uGeom |ydeprecated|");
	_classObj.def_property_readonly("rotOurs", &LawTester::get_rotOurs, "last 3 components of uTest |ydeprecated|");
	_classObj.def_property_readonly("rotGeom", &LawTester::get_rotGeom, "last 3 components of uGeom |ydeprecated|");
	_classObj.def_readwrite("disPath", &LawTester::disPath, "Loading path, where each Vector3 contains desired normal displacement and two components of the shear displacement");
	_classObj.def_readwrite("rotPath", &LawTester::rotPath, "Rotational components of the loading path");
	_classObj.def_readwrite("hooks", &LawTester::hooks, "Python commands to be run when the corresponding point in path is reached");
	_classObj.def_readwrite("uGeom", &LawTester::uGeom, "Current generalized displacements (3 displacements, 3 rotations), as stored in the interaction itself.");
	_classObj.def_readwrite("uTest", &LawTester::uTest, "Current generalized displacements (3 displacements, 3 rotations), as they should be according to this LawTester.");
	_classObj.def_readwrite("uTestNext", &LawTester::uTestNext, "The value of uTest in the next step");
	_classObj.def_readwrite("warnedDeprecPtRot", &LawTester::warnedDeprecPtRot, "Flag to say that the user was already warned about using deprecated ptOurg/ptGeom/rotOurs/rotGeom.");
	_classObj.def_readwrite("shearTot", &LawTester::shearTot, "Current displacement in global coordinates");
	_classObj.def_readwrite("displIsRel", &LawTester::displIsRel, "Whether displacement values in disPath are normalized by reference contact length");
	_classObj.def_readwrite("pathSteps", &LawTester::pathSteps, "Step number for corresponding values in path");
	_classObj.def_readwrite("step", &LawTester::step, "Step number in which this engine is active");
	_classObj.def_readwrite("doneHook", &LawTester::doneHook, "Python command (as string) to run when end of the path is achieved.");
	_classObj.def_readwrite("renderLength", &LawTester::renderLength, "Characteristic length for the purposes of rendering");
	_classObj.def_readwrite("refLength", &LawTester::refLength, "Reference contact length, for rendering only.");
	_classObj.def_readwrite("contPt", &LawTester::contPt, "Contact point (for rendering only)");
	_classObj.def_readwrite("idWeight", &LawTester::idWeight, "Float, usually ∈〈0,1〉, determining on how are displacements distributed between particles");
	_classObj.def_readwrite("rotWeight", &LawTester::rotWeight, "Float ∈〈0,1〉 determining whether shear displacement is applied as rotation or displacement on arc");
}
