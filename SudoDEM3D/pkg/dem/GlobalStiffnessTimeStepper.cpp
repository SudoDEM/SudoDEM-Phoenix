...
void GlobalStiffnessTimeStepper::findTimeStepFromBody(const shared_ptr<Body>& body, Scene * ncb)
{
	...
	if(body->isClump()) {// if clump, we sum stifnesses of all members
		...
		for(Clump::MemberMap::value_type& B :  clump->members){
...
void GlobalStiffnessTimeStepper::computeTimeStep(Scene* ncb)
{
	...
	active=active;
	// TODO: Fix Shop_01.cpp compilation to enable PWaveTimeStep
	// if (defaultDt<0) defaultDt= timestepSafetyCoefficient*Shop::PWaveTimeStep(Omega::instance().getScene());
	if (defaultDt<0) defaultDt = 0.001; // Temporary fix
	...
	LOG_INFO("computed timestep " << newDt <<
			...
			string(", BUT timestep is ")+std::to_string(scene->dt))<<".");
}
...
void GlobalStiffnessTimeStepper::computeStiffnesses(Scene* rb){
	...
	for(const shared_ptr<Interaction>& contact :  *rb->interactions){
...
// pyRegisterClass implementation moved from GlobalStiffnessTimeStepper.hpp
void GlobalStiffnessTimeStepper::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("GlobalStiffnessTimeStepper");
	pybind11::class_<GlobalStiffnessTimeStepper, TimeStepper, std::shared_ptr<GlobalStiffnessTimeStepper>> _classObj(_module, "GlobalStiffnessTimeStepper", "An engine assigning the time-step as a fraction of the minimum eigen-period.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("defaultDt", &GlobalStiffnessTimeStepper::defaultDt, "used as the initial value of the timestep");
	_classObj.def_readwrite("maxDt", &GlobalStiffnessTimeStepper::maxDt, "if positive, used as max value of the timestep");
	_classObj.def_readwrite("previousDt", &GlobalStiffnessTimeStepper::previousDt, "last computed dt");
	_classObj.def_readwrite("timestepSafetyCoefficient", &GlobalStiffnessTimeStepper::timestepSafetyCoefficient, "safety factor between the minimum eigen-period and the final assigned dt");
	_classObj.def_readwrite("densityScaling", &GlobalStiffnessTimeStepper::densityScaling, "density scaling flag");
	_classObj.def_readwrite("targetDt", &GlobalStiffnessTimeStepper::targetDt, "targetDt for density scaling");
	_classObj.def_readwrite("viscEl", &GlobalStiffnessTimeStepper::viscEl, "To use with ViscElPhys");
}
