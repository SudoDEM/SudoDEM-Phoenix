...
shared_ptr<Interaction> Shop::createExplicitInteraction(Body::id_t id1, Body::id_t id2, bool force){
	...
	if(rb->interactions->find(Body::id_t(id1),Body::id_t(id2))!=0) throw runtime_error(string("Interaction #")+std::to_string(id1)+"+#"+std::to_string(id2)+" already exists.");
	for(const shared_ptr<Engine>& e :  rb->engines){
	...
	if(!b1) throw runtime_error(("No body #"+std::to_string(id1)).c_str());
	if(!b2) throw runtime_error(("No body #"+std::to_string(id2)).c_str());
...
py::list Shop::getStressLWForEachBody(){
	...
	for(const Matrix3r& m :  bStresses) ret.append(m);
	return ret;
...
void Shop::setContactFriction(Real angleRad){
	...
	for(const shared_ptr<Interaction>& ii :  *scene->interactions){
...
void Shop::growParticles(Real multiplier, bool updateMass, bool dynamicOnly)
{
	...
	for(const shared_ptr<Body>& b : *scene->bodies){
		...
		if(b->isClump() || sphereIdx == b->shape->getClassIndex()){
