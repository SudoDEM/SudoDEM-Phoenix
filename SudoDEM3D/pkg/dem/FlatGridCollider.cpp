...
void FlatGridCollider::action(){
	if(!newton){
		for(const shared_ptr<Engine>& e :  scene->engines){ newton=SUDODEM_PTR_DYN_CAST<NewtonIntegrator>(e); if(newton) break; }
...
void FlatGridCollider::updateCollisions(){
	...
	for(const Grid::idVector& v :  grid.data){
		size_t sz=v.size();
...
// pyRegisterClass implementation moved from FlatGridCollider.hpp
void FlatGridCollider::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("FlatGridCollider");
	pybind11::class_<FlatGridCollider, Collider, std::shared_ptr<FlatGridCollider>> _classObj(_module, "FlatGridCollider", "Non-optimized grid collider, storing grid as dense flat array.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("verletDist", &FlatGridCollider::verletDist, "Length by which enlarge space occupied by each particle; avoids running collision detection at every step.");
	_classObj.def_readwrite("aabbMin", &FlatGridCollider::aabbMin, "Lower corner of grid.");
	_classObj.def_readwrite("aabbMax", &FlatGridCollider::aabbMax, "Upper corner of grid (approximate, might be rounded up to *minStep*.");
	_classObj.def_readwrite("step", &FlatGridCollider::step, "Step in the grid (cell size)");
}
