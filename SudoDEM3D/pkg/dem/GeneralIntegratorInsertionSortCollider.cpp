...
void GeneralIntegratorInsertionSortCollider::action(){
	...
	// pre-conditions
		...
		if(verletDist>0){
			...
			if(!integrator){
 				for(shared_ptr<Engine>& e :  scene->engines){ integrator=SUDODEM_PTR_DYN_CAST<Integrator>(e); if(integrator) break; }
	...
	static_assert(sizeof(Vector3r)==3*sizeof(Real), "Vector3r must have 3 Reals");
...
void GeneralIntegratorInsertionSortCollider::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("GeneralIntegratorInsertionSortCollider");
	pybind11::class_<GeneralIntegratorInsertionSortCollider, InsertionSortCollider, std::shared_ptr<GeneralIntegratorInsertionSortCollider>> _classObj(_module, "GeneralIntegratorInsertionSortCollider", "This class is the adaptive version of the InsertionSortCollider and changes the NewtonIntegrator dependency of the collider algorithms to the Integrator interface which is more general.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("integrator", &GeneralIntegratorInsertionSortCollider::integrator, "Integrator to use for adaptive stepping");
}
