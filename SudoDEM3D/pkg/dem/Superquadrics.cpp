...
REGISTER_CLASS_INDEX_CPP(Superquadrics,Shape)
REGISTER_CLASS_INDEX_CPP(SuperquadricsGeom,IGeom)
REGISTER_CLASS_INDEX_CPP(SuperquadricsGeom2,SuperquadricsGeom)
REGISTER_CLASS_INDEX_CPP(SuperquadricsMat,Material)
REGISTER_CLASS_INDEX_CPP(SuperquadricsMat2,SuperquadricsMat)
REGISTER_CLASS_INDEX_CPP(SuperquadricsPhys,FrictPhys)
REGISTER_CLASS_INDEX_CPP(SuperquadricsHertzMindlinPhys,SuperquadricsPhys)
...
{
	// std::cerr<<"rrrrrrrxyz "<<rx<<" "<<ry<<" rz"<<rz<<" eps "<<eps1<<" "<<eps2<<std::endl;
	Vector3r Surf;
...
/* Plotting */
	...
	// Note: wire and slices are inline static members defined in the header
...
void Gl1_Superquadrics::initSolidGlList(Superquadrics* t)
{
	...
	double a=0.0,b=0.0;
	double hStep=Mathr::PI/(h-1);
	double wStep=2*Mathr::PI/w;
	...
	for(a=0.0,i=0;i<h;i++,a+=hStep)
	  ...
	  {
		vertices.push_back(t->getSurface(Vector2r(b,a-Mathr::HALF_PI)));
	  }
...
void SuperquadricsGeom2::HertzMindlin(Vector2r curvature1, Vector2r curvature2, double theta){
	...
	//double Fn = 4./3.*M_PI*k*sqrt(E_k/pow(F_k,3)/sum_rho);
...
// pyRegisterClass implementations moved from Superquadrics.hpp
void Superquadrics::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Superquadrics");
	pybind11::class_<Superquadrics, Shape, std::shared_ptr<Superquadrics>> _classObj(_module, "Superquadrics", "Superquadrics shape");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("rxyz", &Superquadrics::rxyz, "length of half-axis");
	_classObj.def_readwrite("rx", &Superquadrics::rx, "length of half-axis x");
	_classObj.def_readwrite("ry", &Superquadrics::ry, "length of half-axis y");
	_classObj.def_readwrite("rz", &Superquadrics::rz, "length of half-axis z");
	_classObj.def_readwrite("eps1", &Superquadrics::eps1, "exponent 1");
	_classObj.def_readwrite("eps2", &Superquadrics::eps2, "exponent 2");
	_classObj.def_readwrite("isSphere", &Superquadrics::isSphere, "the shape is non-spherical by default");
	_classObj.def_readwrite("eps", &Superquadrics::eps, "length of half-axis");
	_classObj.def_readwrite("m_GL_slices", &Superquadrics::m_GL_slices, "GL slices");
	_classObj.def("Initial",&Superquadrics::Initial,"Initialization");
	_classObj.def("getVolume",&Superquadrics::getVolume,"return particle's volume");
	_classObj.def("getrxyz",&Superquadrics::getrxyz,"return particle's rxyz");
	_classObj.def("geteps",&Superquadrics::geteps,"return particle's eps1 and eps2");
	_classObj.def("getInertia",&Superquadrics::getInertia,"return particle's inertia tensor");
	_classObj.def("getOri",&Superquadrics::getOrientation,"return particle's orientation");
	_classObj.def("getCentroid",&Superquadrics::getPosition,"return particle's centroid");
}

void SuperquadricsGeom::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("SuperquadricsGeom");
	pybind11::class_<SuperquadricsGeom, IGeom, std::shared_ptr<SuperquadricsGeom>> _classObj(_module, "SuperquadricsGeom", "Geometry of interaction between Superquadrics");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("PenetrationDepth", &SuperquadricsGeom::PenetrationDepth, "PenetrationDepth");
	_classObj.def_readwrite("contactAngle", &SuperquadricsGeom::contactAngle, "Normal direction (in local coordinates)");
	_classObj.def_readwrite("contactPoint", &SuperquadricsGeom::contactPoint, "Contact point");
	_classObj.def_readwrite("shearInc", &SuperquadricsGeom::shearInc, "Shear displacement increment");
	_classObj.def_readwrite("relativeVn", &SuperquadricsGeom::relativeVn, "relative velocity in the normal");
	_classObj.def_readwrite("relativeVs", &SuperquadricsGeom::relativeVs, "relative velocity in the tangential");
	_classObj.def_readwrite("normal", &SuperquadricsGeom::normal, "Contact normal");
	_classObj.def_readwrite("twist_axis", &SuperquadricsGeom::twist_axis, "Twist axis");
	_classObj.def_readwrite("point1", &SuperquadricsGeom::point1, "point 1");
	_classObj.def_readwrite("point2", &SuperquadricsGeom::point2, "point 2");
	_classObj.def_readwrite("isShearNew", &SuperquadricsGeom::isShearNew, "isShearNew");
	_classObj.def_readwrite("orthonormal_axis", &SuperquadricsGeom::orthonormal_axis, "orthonormal_axis");
}

void SuperquadricsGeom2::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("SuperquadricsGeom2");
	pybind11::class_<SuperquadricsGeom2, SuperquadricsGeom, std::shared_ptr<SuperquadricsGeom2>> _classObj(_module, "SuperquadricsGeom2", "Geometry of interaction between Superquadrics (Hertz-Mindlin)");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("curvatures1", &SuperquadricsGeom2::curvatures1, "Principal curvatures of particle 1");
	_classObj.def_readwrite("curvatures2", &SuperquadricsGeom2::curvatures2, "Principal curvatures of particle 2");
	_classObj.def_readwrite("curvatureAngle", &SuperquadricsGeom2::curvatureAngle, "angle between the two maximum principal curvatures");
	_classObj.def_readwrite("alpha", &SuperquadricsGeom2::alpha, "semi-length ratio of the contact ellipse");
	_classObj.def_readwrite("K", &SuperquadricsGeom2::K, "complete elliptic integral of the first kind");
	_classObj.def_readwrite("E", &SuperquadricsGeom2::E, "complete elliptic integral of the second kind");
	_classObj.def_readwrite("curvatures_sum", &SuperquadricsGeom2::curvatures_sum, "sum of relative curvatures");
	_classObj.def_readwrite("isSphere", &SuperquadricsGeom2::isSphere, "are the two touching bodies spheres");
}

void Bo1_Superquadrics_Aabb::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Bo1_Superquadrics_Aabb");
	pybind11::class_<Bo1_Superquadrics_Aabb, BoundFunctor, std::shared_ptr<Bo1_Superquadrics_Aabb>> _classObj(_module, "Bo1_Superquadrics_Aabb", "Create Aabb from Superquadrics");
	_classObj.def(pybind11::init<>());
}

void SuperquadricsMat::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("SuperquadricsMat");
	pybind11::class_<SuperquadricsMat, Material, std::shared_ptr<SuperquadricsMat>> _classObj(_module, "SuperquadricsMat", "Elastic material with Coulomb friction");
	_classObj.def(pybind11::init<>());
	// Constructor with keyword arguments support
	_classObj.def(pybind11::init([](std::string label, Real Kn, Real Ks, Real frictionAngle, Real density) {
		auto mat = std::make_shared<SuperquadricsMat>();
		mat->label = label;
		mat->Kn = Kn;
		mat->Ks = Ks;
	...
	_classObj.def_readwrite("Kn", &SuperquadricsMat::Kn, "Normal stiffness");
	_classObj.def_readwrite("Ks", &SuperquadricsMat::Ks, "Shear stiffness");
	_classObj.def_readwrite("frictionAngle", &SuperquadricsMat::frictionAngle, "Contact friction angle");
	_classObj.def_readwrite("betan", &SuperquadricsMat::betan, "Normal Damping Ratio");
	_classObj.def_readwrite("betas", &SuperquadricsMat::betas, "Shear Damping Ratio");
	_classObj.def_readwrite("IsSplitable", &SuperquadricsMat::IsSplitable, "To be splitted");
	_classObj.def_readwrite("strength", &SuperquadricsMat::strength, "Stress at which polyhedra breaks");
}

void SuperquadricsMat2::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("SuperquadricsMat2");
	pybind11::class_<SuperquadricsMat2, SuperquadricsMat, std::shared_ptr<SuperquadricsMat2>> _classObj(_module, "SuperquadricsMat2", "Elastic material with Coulomb friction (Hertz-Mindlin)");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("G", &SuperquadricsMat2::G, "shear modulus");
	_classObj.def_readwrite("nu", &SuperquadricsMat2::nu, "Poisson's ratio");
}

void SuperquadricsPhys::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("SuperquadricsPhys");
	pybind11::class_<SuperquadricsPhys, FrictPhys, std::shared_ptr<SuperquadricsPhys>> _classObj(_module, "SuperquadricsPhys", "Simple elastic material with friction for volumetric constitutive laws");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("normalViscous", &SuperquadricsPhys::normalViscous, "Normal viscous component");
	_classObj.def_readwrite("shearViscous", &SuperquadricsPhys::shearViscous, "Shear viscous component");
	_classObj.def_readwrite("betan", &SuperquadricsPhys::betan, "Normal Damping Ratio");
	_classObj.def_readwrite("betas", &SuperquadricsPhys::betas, "Shear Damping Ratio");
}

void SuperquadricsHertzMindlinPhys::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("SuperquadricsHertzMindlinPhys");
	pybind11::class_<SuperquadricsHertzMindlinPhys, SuperquadricsPhys, std::shared_ptr<SuperquadricsHertzMindlinPhys>> _classObj(_module, "SuperquadricsHertzMindlinPhys", "Hertz-Mindlin physics for Superquadrics");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("nuab", &SuperquadricsHertzMindlinPhys::nuab, "average nu of two particles");
	_classObj.def_readwrite("Gab", &SuperquadricsHertzMindlinPhys::Gab, "Shear modulus");
	_classObj.def_readwrite("mu1", &SuperquadricsHertzMindlinPhys::mu1, "parameter");
	_classObj.def_readwrite("mu2", &SuperquadricsHertzMindlinPhys::mu2, "parameter");
}

void Ip2_SuperquadricsMat_SuperquadricsMat_SuperquadricsPhys::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Ip2_SuperquadricsMat_SuperquadricsMat_SuperquadricsPhys");
	pybind11::class_<Ip2_SuperquadricsMat_SuperquadricsMat_SuperquadricsPhys, IPhysFunctor, std::shared_ptr<Ip2_SuperquadricsMat_SuperquadricsMat_SuperquadricsPhys>> _classObj(_module, "Ip2_SuperquadricsMat_SuperquadricsMat_SuperquadricsPhys", "Material to physics functor for Superquadrics");
	_classObj.def(pybind11::init<>());
}

void Ip2_SuperquadricsMat2_SuperquadricsMat2_HertzMindlinPhys::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Ip2_SuperquadricsMat2_SuperquadricsMat2_HertzMindlinPhys");
	pybind11::class_<Ip2_SuperquadricsMat2_SuperquadricsMat2_HertzMindlinPhys, IPhysFunctor, std::shared_ptr<Ip2_SuperquadricsMat2_SuperquadricsMat2_HertzMindlinPhys>> _classObj(_module, "Ip2_SuperquadricsMat2_SuperquadricsMat2_HertzMindlinPhys", "Material to physics functor for Hertz-Mindlin");
	_classObj.def(pybind11::init<>());
}

void SuperquadricsLaw::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("SuperquadricsLaw");
	pybind11::class_<SuperquadricsLaw, LawFunctor, std::shared_ptr<SuperquadricsLaw>> _classObj(_module, "SuperquadricsLaw", "Calculate physical response of Superquadrics");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("shearForce", &SuperquadricsLaw::shearForce, "Shear force from last step");
	_classObj.def_readwrite("traceEnergy", &SuperquadricsLaw::traceEnergy, "Define the total energy dissipated in plastic slips");
	_classObj.def("elasticEnergy", &SuperquadricsLaw::elasticEnergy, "Compute and return the total elastic energy");
	_classObj.def("getPlasticDissipation", &SuperquadricsLaw::getPlasticDissipation, "Total energy dissipated in plastic slips");
	_classObj.def("initPlasticDissipation", &SuperquadricsLaw::initPlasticDissipation, "Initialize cummulated plastic dissipation");
}

void SuperquadricsLaw2::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("SuperquadricsLaw2");
	pybind11::class_<SuperquadricsLaw2, LawFunctor, std::shared_ptr<SuperquadricsLaw2>> _classObj(_module, "SuperquadricsLaw2", "Calculate physical response of Superquadrics (Hertz-Mindlin)");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("shearForce", &SuperquadricsLaw2::shearForce, "Shear force from last step");
}

#ifdef SUDODEM_OPENGL
void Gl1_Superquadrics::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Gl1_Superquadrics");
	pybind11::class_<Gl1_Superquadrics, Functor, std::shared_ptr<Gl1_Superquadrics>> _classObj(_module, "Gl1_Superquadrics", "Renders :yref:`Superquadrics` object");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite_static("wire", &Gl1_Superquadrics::wire, "Render as wireframe");
	_classObj.def_readwrite_static("slices", &Gl1_Superquadrics::slices, "Number of slices for rendering");
}
#endif
