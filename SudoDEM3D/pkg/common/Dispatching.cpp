...
{
	// std::cerr << "DEBUG BoundDispatcher::action: scene=" << scene << std::endl;
	updateScenePtr();
	// std::cerr << "DEBUG BoundDispatcher::action: updateScenePtr done" << std::endl;
	shared_ptr<BodyContainer>& bodies = scene->bodies;
	const long numBodies=(long)bodies->size();
	// std::cerr << "DEBUG BoundDispatcher::action: numBodies=" << numBodies << std::endl;
	...
	// std::cerr << "DEBUG BoundDispatcher::action: completed" << std::endl;
...
// 	const shared_ptr<Body>& b=(*bodies)[id];
		...
		static int count = 0;
		// if (count++ < 5) {
		// 	std::cerr << "DEBUG processBody: shape=" << shape->getClassName() << " scene=" << scene << std::endl;
		// }
		if(b->bound) {
		...
		shape->boundFunctor->scene=scene; // FIX: update scene pointer for cached functor
...
void IPhysDispatcher::action()
{
	...
	#else
		for(const shared_ptr<Interaction>& interaction : *scene->interactions){
	#endif
			if(interaction->geom){
