...
void InsertionSortCollider::pyRegisterClass(pybind11::module_ _module) {
	...
	// Constructor with BoundFunctor list and keyword arguments
	_classObj.def(pybind11::init([](pybind11::list boundFunctors, pybind11::kwargs kwargs) {
		auto collider = std::make_shared<InsertionSortCollider>();
		// Process BoundFunctor list
		typedef std::vector<shared_ptr<BoundFunctor>> vecBound;
		vecBound vb = pybind11::cast<vecBound>(boundFunctors);
		for (auto bf : vb) collider->boundDispatcher->add(bf);
		// Process keyword arguments
		for (auto item : kwargs) {
			std::string key = pybind11::cast<std::string>(item.first);
			if (key == "verletDist") {
				collider->verletDist = pybind11::cast<Real>(item.second);
			}
			// Add more keyword arguments as needed
		}
		return collider;
	}), pybind11::arg("boundFunctors") = pybind11::list());
	_classObj.def_readwrite("sortAxis", &InsertionSortCollider::sortAxis, "Axis for the initial contact detection.");
	_classObj.def_readwrite("allowBiggerThanPeriod", &InsertionSortCollider::allowBiggerThanPeriod, "If true, tests on bodies sizes will be disabled, and the simulation will run normaly even if bodies larger than period are found. It can be useful when the periodic problem include e.g. a floor modelized with wall/box/facet.\nBe sure you know what you are doing if you touch this flag. The result is undefined if one large body moves out of the (0,0,0) period.");
	_classObj.def_readwrite("sortThenCollide", &InsertionSortCollider::sortThenCollide, "Separate sorting and colliding phase; it is MUCH slower, but all interactions are processed at every step; this effectively makes the collider non-persistent, not remembering last state. (The default behavior relies on the fact that inversions during insertion sort are overlaps of bounding boxes that just started/ceased to exist, and only processes those; this makes the collider much more efficient.)");
	_classObj.def_readwrite("targetInterv", &InsertionSortCollider::targetInterv, "(experimental) Target number of iterations between bound update, used to define a smaller sweep distance for slower grains if >0, else always use 1*verletDist. Useful in simulations with strong velocity contrasts between slow bodies and fast bodies.");
	_classObj.def_readwrite("updatingDispFactor", &InsertionSortCollider::updatingDispFactor, "(experimental) Displacement factor used to trigger bound update: the bound is updated only if updatingDispFactor*disp>sweepDist when >0, else all bounds are updated.");
	_classObj.def_readwrite("verletDist", &InsertionSortCollider::verletDist, "Length by which to enlarge particle bounds, to avoid running collider at every step. Stride disabled if zero. Negative value will trigger automatic computation, so that the real value will be *verletDist* × minimum spherical particle radius; if there are no spherical particles, it will be disabled. The actual length added to one bound can be only a fraction of verletDist when :yref:`InsertionSortCollider::targetInterv` is > 0.");
	_classObj.def_readwrite("minSweepDistFactor", &InsertionSortCollider::minSweepDistFactor, "Minimal distance by which enlarge all bounding boxes; superseeds computed value of verletDist when lower that (minSweepDistFactor x verletDist).");
	_classObj.def_readwrite("fastestBodyMaxDist", &InsertionSortCollider::fastestBodyMaxDist, "Normalized maximum displacement of the fastest body since last run; if >= 1, we could get out of bboxes and will trigger full run. |yupdate|");
	_classObj.def_readwrite("numReinit", &InsertionSortCollider::numReinit, "Cummulative number of bound array re-initialization.");
	_classObj.def_readwrite("useless", &InsertionSortCollider::useless, "for compatibility of scripts defining the old collider's attributes - see deprecated attributes");
	_classObj.def_readwrite("doSort", &InsertionSortCollider::doSort, "Do forced resorting of interactions.");
	_classObj.def_readonly("strideActive", &InsertionSortCollider::strideActive, "Whether striding is active (read-only; for debugging). |yupdate|");
	_classObj.def_readonly("periodic", &InsertionSortCollider::periodic, "Whether the collider is in periodic mode (read-only; for debugging) |yupdate|");
	_classObj.def("dumpBounds", &InsertionSortCollider::dumpBounds, "Return representation of the internal sort data. The format is ``([...],[...],[...])`` for 3 axes, where each ``...`` is a list of entries (bounds). The entry is a tuple with the fllowing items:\n\n* coordinate (float)\n* body id (int), but negated for negative bounds\n* period numer (int), if the collider is in the periodic regime.");
}
...
void InsertionSortCollider::insertionSortParallel(VecBounds& v, InteractionContainer* interactions, Scene*, bool doCollide){
	...
	for (int k=1; k<nChunks;k++) if (v[chunks[k]]<v[chunks[k]-1]) {
       ...
       invSizes=Vector3r(1./scene->cell->getSize()[0],1./scene->cell->getSize()[1],1./scene->cell->getSize()[2]);
		...
		if(verletDist<0){
			...
			for(const shared_ptr<Body>& b : *scene->bodies){
				...
				Sphere* s=dynamic_cast<Sphere*>(b->shape.get());//fixme for other shapes
				if(!s) continue;
