...
void FEMdriver::driveNodes(){
	...
	SUDODEM_PARALLEL_FOREACH_NODE_BEGIN(const shared_ptr<Node>& b, scene->nodes){
			...
			#ifdef SUDODEM_DEBUG
				if(isnan(f[0])||isnan(f[1])||isnan(f[2])) throw runtime_error(("FEMdriver: NaN force acting on #"+std::to_string(id)+".").c_str());
				if(isnan(m[0])||isnan(m[1])||isnan(m[2])) throw runtime_error(("FEMdriver: NaN torque acting on #"+std::to_string(id)+".").c_str());
				if(b->mass<=0 && ((b->blockedDOFs & Node::DOF_XYZ) != Node::DOF_XYZ)) throw runtime_error(("FEMdriver: #"+std::to_string(id)+" has some linear accelerations enabled, but State::mass is non-positive."));
				if(b->inertia.minCoeff()<=0 && ((b->blockedDOFs & Node::DOF_RXRYRZ) != Node::DOF_RXRYRZ)) throw runtime_error(("FEMdriver: #"+std::to_string(id)+" has some angular accelerations enabled, but inertia contains non-positive terms."));
			#endif
	        ...
	        if (angle2!=0 && ( (mask<=0) || ((mask>0) && (Node::byId(id)->maskCompatible(mask))) )) {//If we have an angular velocity, we make a rotation
	        ...
	        if(scene->nodeforces.getMoveRotUsed() && scene->nodeforces.getRot(id)!=Vector3r::Zero()
		        && ( (mask<=0) || ((mask>0) && (Node::byId(id)->maskCompatible(mask))) )) {
	        ...
	        if ( (mask<=0) || ((mask>0) && (Node::byId(id)->maskCompatible(mask))) ) {
		        b->pos+=b->vel*dt;
	        }    
...
void FEMdriver::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("FEMdriver");
	pybind11::class_<FEMdriver, FieldApplier, std::shared_ptr<FEMdriver>> _classObj(_module, "FEMdriver", "Engine integrating newtonian motion equations.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("damping", &FEMdriver::damping, "damping coefficient for Cundall's non viscous damping");
	_classObj.def_readwrite("bending", &FEMdriver::bending, "is TriElement bending?");
	_classObj.def_readwrite("rotIncr", &FEMdriver::rotIncr, "is TriElement rotation increment?");
	_classObj.def_readwrite("thickness", &FEMdriver::thickness, "Element thickness");
	_classObj.def_readwrite("young", &FEMdriver::young, "Young's modulus");
	_classObj.def_readwrite("nu", &FEMdriver::nu, "Poisson's ratio");
	_classObj.def_readwrite("bendThickness", &FEMdriver::bendThickness, "Element bend thickness");
	_classObj.def_readwrite("gravity", &FEMdriver::gravity, "Gravitational acceleration");
	_classObj.def_readwrite("maxVelocitySq", &FEMdriver::maxVelocitySq, "store square of max. velocity");
	_classObj.def_readwrite("warnNoForceReset", &FEMdriver::warnNoForceReset, "Warn when forces were not resetted");
	_classObj.def_readwrite("mask", &FEMdriver::mask, "Body mask");
}
