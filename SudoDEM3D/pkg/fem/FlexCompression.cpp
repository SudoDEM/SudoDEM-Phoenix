...
#include<sudodem/pkg/fem/FlexCompression.hpp>
    ...
    //get the volume of all solid particles
		...
		for ( ; bi!=biEnd; ++bi )
		{
            ...
            if (b->shape->getClassName()=="Sphere"){
				...
				p_v += 4.0/3.0*Mathr::PI*pow(A->radius,3.0);
...
void FlexCompressionEngine::rigid_consolidate(){
    ...
    if (scene->iter % echo_interval == 0){
	    ...
	    <<", Ss_z:"<<wszz/1000.0
      <<", e:" << ((Mathr::PI*pow(wall_radius,2.0)*height)/particlesVolume - 1.0)
	    <<"\n";
    }
...
double FlexCompressionEngine::get_boxVolume(){
    ...
    if(isFlexible){//use a flexible wall
        ...
        box_v += height*Mathr::PI*pow(wall_radius,2.0)/3.0;//FIXME:the top and bottom ends are not exactly circular.
        ...
        return height*Mathr::PI*pow(wall_radius,2.0);
    }
}
...
void FlexCompressionEngine::recordData()
{
	...
	//out << std::to_string(scene->iter) << " "
 	//<< std::to_string(strain[0]) << " "  //
 	//<< std::to_string(strain[1]) << " "  //
	out << std::to_string(strain[2]) << " "
	<< std::to_string(log(Init_boxVolume/Current_boxVolume)) << " " //volumetric strain
 	<< std::to_string(pq[0]) << " "
    << std::to_string(pq[1]) << " "
...
void FlexCompressionEngine::get_gainz(){//determine servo gain parameters
   //cylindrical wall
	...
	//calc gains
    if(isFlexible){
        gain_r = 2.0*gain_alpha*height*Mathr::PI*2.0*wall_radius/(avg_rstiff);
    }else{
        gain_r = 2.0*gain_alpha*height*Mathr::PI*2.0*wall_radius/(num_pwall*1e8);
    }
...
void FlexCompressionEngine::consol_ss_cylinder(){
    ...
    x_area = 2.0*Mathr::PI*wall_radius * height;
	loading_area = Mathr::PI*pow(wall_radius,2.0);
    if(debugMode){
        x_area = 0.0;
        for(vector<shared_ptr<TriElement>>::iterator elm=Elements.begin();elm!=Elements.end();++elm){
...
void FlexCompressionEngine::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("FlexCompressionEngine");
	pybind11::class_<FlexCompressionEngine, BoundaryController, std::shared_ptr<FlexCompressionEngine>> _classObj(_module, "FlexCompressionEngine", "An engine maintaining constant stresses or constant strain rates on some boundaries of a parallepipedic packing.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("wall_radius", &FlexCompressionEngine::wall_radius, "the radius of the cylindrical wall");
	_classObj.def_readwrite("wall_fix", &FlexCompressionEngine::wall_fix, "the radius of the cylindrical wall");
	_classObj.def_readwrite("gain_alpha", &FlexCompressionEngine::gain_alpha, "alpha during geting gains");
	_classObj.def_readwrite("target_strain", &FlexCompressionEngine::target_strain, "the target axial strain during shear");
	_classObj.def_readwrite("iterate_num_max", &FlexCompressionEngine::iterate_num_max, "interval of restart counting iterate_num");
	_classObj.def_readwrite("solve_num_max", &FlexCompressionEngine::solve_num_max, "interval of restart counting solve_num");
	_classObj.def_readwrite("z_servo", &FlexCompressionEngine::z_servo, "stress control or strain control in the z direction");
	_classObj.def_readwrite("isFlexible", &FlexCompressionEngine::isFlexible, "use the flexible membrane?");
	_classObj.def_readwrite("keepRigid", &FlexCompressionEngine::keepRigid, "keep the flexible wall rigid? Usually used during consolidation.");
	_classObj.def_readwrite("stiffnessUpdateInterval", &FlexCompressionEngine::stiffnessUpdateInterval, "interval of stiffness updating at walls");
	_classObj.def_readwrite("echo_interval", &FlexCompressionEngine::echo_interval, "interval of iteration for printing run info.");
	_classObj.def_readwrite("ramp_interval", &FlexCompressionEngine::ramp_interval, "chunck bins for accelerating walls gradually.");
	_classObj.def_readwrite("ramp_chunks", &FlexCompressionEngine::ramp_chunks, "chuncks for accelerating walls gradually.");
	_classObj.def_readwrite("stressMask", &FlexCompressionEngine::stressMask, "constant confining stress (non-zero) or contant volume (0)");
	_classObj.def_readwrite("bottom_wall_activated", &FlexCompressionEngine::bottom_wall_activated, "if true, this wall moves according to the target value");
	_classObj.def_readwrite("top_wall_activated", &FlexCompressionEngine::top_wall_activated, "if true, this wall moves according to the target value");
	_classObj.def_readwrite("file", &FlexCompressionEngine::file, "Name of file to save to");
	_classObj.def_readwrite("height", &FlexCompressionEngine::height, "size of the box z(2-axis)");
	_classObj.def_readwrite("f_threshold", &FlexCompressionEngine::f_threshold, "threshold for approaching the target fast.");
	_classObj.def_readwrite("fmin_threshold", &FlexCompressionEngine::fmin_threshold, "threshold for stresses on the walls relaxation.");
	_classObj.def_readwrite("alpha", &FlexCompressionEngine::alpha, "gain for stress control");
	_classObj.def_readwrite("wall_radius0", &FlexCompressionEngine::wall_radius0, "the initial radius of the cylinderical wall");
	_classObj.def_readwrite("height0", &FlexCompressionEngine::height0, "Reference size for strain definition.");
	_classObj.def_readwrite("wall_equal_tol", &FlexCompressionEngine::wall_equal_tol, "wall equilibrium tolerance after each increament.");
	_classObj.def_readwrite("majorF_tol", &FlexCompressionEngine::majorF_tol, "stress gradual reduction torlerance of top and bottom walls when compressing");
	_classObj.def_readwrite("unbf_tol", &FlexCompressionEngine::unbf_tol, "tolerance of UnbalancedForce");
	_classObj.def_readwrite("wall_max_vel", &FlexCompressionEngine::wall_max_vel, "max velocity of the top facet when loading");
	_classObj.def_readwrite("step_interval", &FlexCompressionEngine::step_interval, "step interval during each displacement increament");
	_classObj.def_readwrite("Sphere_on", &FlexCompressionEngine::Sphere_on, "true for spherical particles, false for polyhedral particles");
	_classObj.def_readwrite("hydroStrain", &FlexCompressionEngine::hydroStrain, "this option is used to perform a hydro strain rate on all directions");
	_classObj.def_readwrite("hydroStrainRate", &FlexCompressionEngine::hydroStrainRate, "this option is used to perform a hydro strain rate on all directions");
	_classObj.def_readwrite("debugMode", &FlexCompressionEngine::debugMode, "this option is used to output debugMode info");
	_classObj.def_readwrite("goalr", &FlexCompressionEngine::goalr, "prescribed stress/strain rate in the radial direction");
	_classObj.def_readwrite("goalz", &FlexCompressionEngine::goalz, "prescribed stress/strain rate on axis z");
	_classObj.def_readwrite("total_incr", &FlexCompressionEngine::total_incr, "the total increaments");
	_classObj.def_readwrite("outinfo_interval", &FlexCompressionEngine::outinfo_interval, "the iteration interval at which displaying the info for monitoring");
	_classObj.def_readwrite("savedata_interval", &FlexCompressionEngine::savedata_interval, "the interval steps between two savedata operations");
	_classObj.def_readwrite("max_vel", &FlexCompressionEngine::max_vel, "Maximum allowed walls velocity");
	_classObj.def_readwrite("externalWork", &FlexCompressionEngine::externalWork, "Energy provided by boundaries.");
	_classObj.def_readwrite("shearRate", &FlexCompressionEngine::shearRate, "target shear rate in direction x");
	_classObj.def_readwrite("currentStrainRate2", &FlexCompressionEngine::currentStrainRate2, "current strain rate in direction 2");
	_classObj.def_readwrite("UnbalancedForce", &FlexCompressionEngine::UnbalancedForce, "mean resultant forces divided by mean contact force");
	_classObj.def_readwrite("frictionAngleDegree", &FlexCompressionEngine::frictionAngleDegree, "Value of friction used in the simulation");
	_classObj.def_readwrite("updateFrictionAngle", &FlexCompressionEngine::updateFrictionAngle, "Switch to activate the update of the intergranular friction");
	_classObj.def_readwrite("Confining", &FlexCompressionEngine::Confining, "Switch to confine or shear");
	_classObj.def_readwrite("Key", &FlexCompressionEngine::Key, "A string appended at the end of all files");
	_classObj.def_readwrite("porosity", &FlexCompressionEngine::porosity, "Porosity of the packing.");
	_classObj.def("getStrain", [](FlexCompressionEngine& self) {
		py::list result;
		for(int i=0; i<3; i++) result.append(self.strain[i]);
		return result;
	}, "Current strain in a vector (exx,eyy,ezz).");
	_classObj.def("getStress", &FlexCompressionEngine::getStress, "get stress within the assembly.");
	_classObj.def("getBoxVolume", &FlexCompressionEngine::get_boxVolume, "get the specimen volume.");
	_classObj.def("fix_wall", &FlexCompressionEngine::fix_wall, "set no degree of freedom for all nodes.");
	_classObj.def("free_wall", &FlexCompressionEngine::free_wall, "set XY DOF of the nodes contacting with the walls, and free other nodes.");
	_classObj.def("getParticleVolume", &FlexCompressionEngine::get_particleVolume, "get the volume of all particles within the specimen.");
}
