...
// Destructor definition - required for typeinfo generation
TriElement::~TriElement() {}

REGISTER_CLASS_INDEX_CPP(TriElement,Shape)
...
void TriElement::postLoad(TriElement&)
{
	...
	if(vertices.size()!=3){ throw runtime_error(("TriElement must have exactly 3 vertices (not "+std::to_string(vertices.size())+")").c_str()); }
...
void TriElement::computeNodalDisplacements(Real dt, bool rotIncr){
	...
	for(int i:{0,1,2}){
		...
		if(rotIncr){
			...
			*/
			if(aa.angle()>Mathr::PI) aa.angle()-=2*Mathr::PI;
...
void TriElement::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("TriElement");
	pybind11::class_<TriElement, Shape, std::shared_ptr<TriElement>> _classObj(_module, "TriElement", "TriElement (triangular particle) geometry.");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("nodes", &TriElement::nodes, "Node instance associated with this Element.");
	_classObj.def_readwrite("ori", &TriElement::ori, "Orientation of this node.");
	_classObj.def_readwrite("vertices", &TriElement::vertices, "Vertex positions in local coordinates.");
	_classObj.def_readwrite("normal", &TriElement::normal, "TriElement's normal (in local coordinate system)");
	_classObj.def_readwrite("area", &TriElement::area, "TriElement's area");
	_classObj.def_readwrite("halfThick", &TriElement::halfThick, "Geometric thickness (added in all directions)");
	_classObj.def_readwrite("pos", &TriElement::pos, "Position.");
	_classObj.def_readwrite("refRot", &TriElement::refRot, "Rotation applied to nodes to obtain the local coordinate system.");
	_classObj.def_readwrite("refPos", &TriElement::refPos, "Nodal coordinates in the local coordinate system, in the reference configuration");
	_classObj.def_readwrite("uXy", &TriElement::uXy, "Nodal displacements.");
	_classObj.def_readwrite("surfLoad", &TriElement::surfLoad, "Normal load applied to this facet.");
	_classObj.def_readwrite("phiXy", &TriElement::phiXy, "Nodal rotations.");
	_classObj.def_readwrite("KKcst", &TriElement::KKcst, "Stiffness matrix of the element.");
	_classObj.def_readwrite("KKdkt", &TriElement::KKdkt, "Bending stiffness matrix of the element.");
	_classObj.def("setRefConf", &TriElement::setRefConf, "Set the current configuration as the reference one.");
	_classObj.def("update", &TriElement::stepUpdate, "Update current configuration.");
	_classObj.def("reset", &TriElement::pyReset, "Reset reference configuration.");
}

#ifdef SUDODEM_OPENGL
void Gl1_TriElement::pyRegisterClass(pybind11::module_ _module) {
	checkPyClassRegistersItself("Gl1_TriElement");
	pybind11::class_<Gl1_TriElement, GlShapeFunctor, std::shared_ptr<Gl1_TriElement>> _classObj(_module, "Gl1_TriElement", "Renders TriElement object");
	_classObj.def(pybind11::init<>());
	_classObj.def_readwrite("wire", &Gl1_TriElement::wire, "Only show wireframe");
	_classObj.def_readwrite("normals", &Gl1_TriElement::normals, "In wire mode, render normals of facets and edges.");
}
#endif
