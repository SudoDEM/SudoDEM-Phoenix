...
class DynLibDispatcher
{
	...
	typedef typename Impl::Parm15 Parm15;
 	...
 	public:
		...
		// Update scene pointer for all functors in the dispatch matrix
		// Overload for 1D dispatchers (callBacks is vector<shared_ptr<Executor>>)
		template<typename T = MatrixType>
		typename std::enable_if<std::is_same<T, std::vector<std::shared_ptr<Executor>>>::value>::type
		updateScenePtr(Scene* scene){
			for(auto& cb : callBacks){ if(cb) cb->scene = scene; }
		}
		
		// Overload for 2D dispatchers (callBacks is vector<vector<shared_ptr<Executor>>>)
		template<typename T = MatrixType>
		typename std::enable_if<std::is_same<T, std::vector<std::vector<std::shared_ptr<Executor>>>>::value>::type
		updateScenePtr(Scene* scene){
			for(auto& cbRow : callBacks){ for(auto& cb : cbRow){ if(cb) cb->scene = scene; } }
		}

		shared_ptr<Executor> getExecutor(shared_ptr<BaseClass1>& arg1){
		  	int ix1;
			if(arg1->getClassIndex()<0) throw runtime_error("No functor for type "+arg1->getClassName()+" (index "+std::to_string(arg1->getClassIndex())+"), since the index is invalid (negative).");
		...
		shared_ptr<Executor> getExecutor(shared_ptr<BaseClass1>& arg1, shared_ptr<BaseClass2>& arg2){
			if(arg1->getClassIndex()<0 || arg2->getClassIndex()<0) throw runtime_error("No functor for types "+arg1->getClassName()+" (index "+std::to_string(arg1->getClassIndex())+") + "+arg2->getClassName()+" (index "+std::to_string(arg2->getClassIndex())+"), since some of the indices is invalid (negative).");
			int ix1,ix2;
 	...
 	public:
		// Template-based version: uses direct instantiation instead of factory
		// This eliminates the need for ClassRegistry factory
		template<typename BaseClassType>
		void add1DEntry(shared_ptr<Executor> executor){
			// Create base class directly using make_shared
			auto baseClass = std::make_shared<BaseClassType>();
			
			// Get indexable interface
			shared_ptr<Indexable> base = std::dynamic_pointer_cast<Indexable>(baseClass);

			if (!base) {
				std::cerr << "ERROR: Failed to cast " << typeid(BaseClassType).name() << " to Indexable" << std::endl;
				return;
			}
		...
		// Template-based version: uses direct instantiation instead of factory
		template<typename BaseClass1Type, typename BaseClass2Type>
		void add2DEntry(shared_ptr<Executor> executor){
			// Create base classes directly using make_shared
			auto baseClass1 = std::make_shared<BaseClass1Type>();
			auto baseClass2 = std::make_shared<BaseClass2Type>();
			
			shared_ptr<Indexable> base1 = std::dynamic_pointer_cast<Indexable>(baseClass1);
			shared_ptr<Indexable> base2 = std::dynamic_pointer_cast<Indexable>(baseClass2);
			...
			callBacks.resize( maxCurrentIndex1+1 );
			for(size_t i=0; i<callBacks.size(); i++) callBacks[i].resize( maxCurrentIndex2+1 );
			callBacks[index1][index2] = executor;
		};

		// Legacy string-based version (kept for backward compatibility)
		void add1DEntry(string baseClassName, shared_ptr<Executor> executor){
			...
			// Use ClassRegistry to create the base class
			shared_ptr<Factorable> obj = ClassRegistry::instance().create(baseClassName);
			if (obj) {
				baseClass = SUDODEM_PTR_CAST<BaseClass1>(obj);
			}
			
			if (!baseClass) {
				std::cerr << "ERROR: Failed to create base class " << baseClassName << " for dispatch." << std::endl;
				return;
			}
			...
			if (!base) {
				std::cerr << "ERROR: Failed to cast " << baseClassName << " to Indexable" << std::endl;
				return;
			}
	...
	public:
		void add2DEntry(string baseClassName1, string baseClassName2, shared_ptr<Executor> executor){
			// Use ClassRegistry to create base classes
			shared_ptr<Factorable> obj1 = ClassRegistry::instance().create(baseClassName1);
			shared_ptr<Factorable> obj2 = ClassRegistry::instance().create(baseClassName2);
			shared_ptr<BaseClass1> baseClass1 = SUDODEM_PTR_CAST<BaseClass1>(obj1);
			shared_ptr<BaseClass2> baseClass2 = SUDODEM_PTR_CAST<BaseClass2>(obj2);
