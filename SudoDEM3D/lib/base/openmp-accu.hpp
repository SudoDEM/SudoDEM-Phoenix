...
// _SC_LEVEL1_DCACHE_LINESIZE is not available on macOS, use default 64 bytes
#ifndef _SC_LEVEL1_DCACHE_LINESIZE
#define _SC_LEVEL1_DCACHE_LINESIZE 0
#endif
...
class OpenMPAccumulator{
	...
	// .def_property_readonly    ("myAccu",&OpenMPAccumulator::get,"documentation")
...
// Cereal serialization
	template<class Archive> void serialize(Archive &ar, OpenMPAccumulator<int>& a){ int value; if(Archive::is_saving::value) value=a.get(); ar(CEREAL_NVP(value)); if(Archive::is_loading::value) a.set(value); }
	template<class Archive> void serialize(Archive &ar, OpenMPAccumulator<Real>& a){ Real value; if(Archive::is_saving::value) value=a.get(); ar(CEREAL_NVP(value)); if(Archive::is_loading::value) a.set(value); }
	template<class Archive> void save(Archive &ar, const OpenMPArrayAccumulator<Real>& a){ size_t size=a.size(); ar(CEREAL_NVP(size)); for(size_t i=0; i<size; i++) { Real item(a.get(i)); ar(CEREAL_NVP(item)); } }
	template<class Archive> void load(Archive &ar, OpenMPArrayAccumulator<Real>& a){ size_t size; ar(CEREAL_NVP(size)); a.resize(size); for(size_t i=0; i<size; i++){ Real item; ar(CEREAL_NVP(item)); a.set(i,item); } }
