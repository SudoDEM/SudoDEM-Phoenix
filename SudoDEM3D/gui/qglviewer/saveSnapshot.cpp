...
{
	bool ok = false;
	QStringList list = formats.split(";;", QT_SKIP_EMPTY_PARTS);
...
bool QGLViewer::saveImageSnapshot(const QString& fileName)
{
	...
	for (int i=0; i<nbX; i++)
		for (int j=0; j<nbY; j++)
		{
			...
			QImage snapshot = QGLWIDGET_grabFrameBuffer(this);
...
void QGLViewer::saveSnapshot(bool automatic, bool overwrite)
{
	...
	if ((automatic) && (snapshotCounter() >= 0))
	{
		...
		QString count;
		count = QString::asprintf("%.04d", snapshotCounter_++);
		QString suffix;
...
QImage QGLViewer::frameBufferSnapshot()
{
	...
	return QGLWIDGET_grabFrameBuffer(this);
}
