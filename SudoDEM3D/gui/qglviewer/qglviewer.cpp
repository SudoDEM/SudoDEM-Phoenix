...
void QGLViewer::postDraw()
{
	...
	{
		f_p_s_ = 1000.0 * maxCounter / QTIME_RESTART(fpsTime_);
...
void QGLViewer::setDefaultMouseBindings()
{
	...
	for (int handler=0; handler<2; ++handler)
	{
		...
		setMouseBinding(modifiers, Qt_MidButton,   mh, ZOOM);
	...
	setMouseBinding(Qt::ShiftModifier, Qt_MidButton, CAMERA, ZOOM_ON_REGION);
	...
	setMouseBinding(Qt::NoModifier, Qt_MidButton,  SHOW_ENTIRE_SCENE, true);
...
void QGLViewer::drawText(int x, int y, const QString& text, const QFont& fnt)
{
	...
	if (tileRegion_ != NULL) {
		::renderText(this, int((x-tileRegion_->xMin) * width() / (tileRegion_->xMax - tileRegion_->xMin)),
				   int((y-tileRegion_->yMin) * height() / (tileRegion_->yMax - tileRegion_->yMin)), text, fnt);
	} else
		::renderText(this, x, y, text, fnt);
}
...
static QString mouseButtonsString(Qt::MouseButtons b)
{
	...
	if (b & Qt_MidButton)     { if (addAmpersand) result += " & "; result += QGLViewer::tr("Middle", "middle mouse button"); addAmpersand=true; }
...
QString QGLViewer::mouseActionString(QGLViewer::MouseAction ma)
{
	switch (ma)
	{
		case QGLViewer::NO_MOUSE_ACTION : return QString_null;
...
QString QGLViewer::clickActionString(QGLViewer::ClickAction ca)
{
	switch (ca)
	{
		case QGLViewer::NO_CLICK_ACTION : return QString_null;
...
\p button is one of the Qt::MouseButtons (\c Qt::LeftButton, \c Qt_MidButton,
\c Qt::RightButton...).
...
void QGLViewer::keyPressEvent(QKeyEvent *e)
{
	...
	if (it != end)
		...
		if (pathIndex_.contains(Qt::Key(key)))
		{
			...
			{
				int elapsed = QTIME_RESTART(doublePress);
...
void QGLViewer::toggleCameraMode()
{
	...
	if (rotateMode)
	{
		...
		setMouseBinding(modifiers, Qt_MidButton,   CAMERA, LOOK_AROUND);
		...
		setMouseBinding(Qt::NoModifier, Qt_MidButton,   NO_CLICK_ACTION, true);
	...
	else
	{
		...
		setMouseBinding(modifiers, Qt_MidButton,   CAMERA, ZOOM);
...
Setting this value to \c QString_null will disable the automatic state file saving that normally
occurs on exit.
...
upper right \c x close button). setStateFileName() to \c QString_null to prevent this. */
