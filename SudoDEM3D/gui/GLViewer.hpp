...
class GLViewer : public QGLViewer
{
	...
	private :
		...
		string strBoundGroup(){string ret;for(int i : boundClipPlanes) ret+=" "+std::to_string(i+1);return ret;}
		std::chrono::system_clock::time_point last_user_event;
     ...
     public:
		...
		void initFromDOMElement(const QDomElement& element);
				...
				//! Enable or disable OpenGL updates (to save CPU when viewer is hidden)

				void enableUpdates(bool enabled) { updatesEnabled = enabled; }

				//! Check if OpenGL updates are enabled

				bool isUpdatesEnabled() const { return updatesEnabled; }
				void setNextFrameSnapshotFilename(const string& filename) { nextFrameSnapshotFilename = filename; }
				const string& getNextFrameSnapshotFilename() const { return nextFrameSnapshotFilename; }


			private:

				bool updatesEnabled = true;  //!< Flag to control OpenGL updates
		...
		std::chrono::system_clock::time_point getLastUserEvent();
