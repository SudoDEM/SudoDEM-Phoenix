...
void GLViewer::useDisplayParameters(size_t n){
	...
	if(dispParams.size()<=(size_t)n){ throw std::invalid_argument(("Display parameters #"+std::to_string(n)+" don't exist (number of entries "+std::to_string(dispParams.size())+")").c_str());; return;}
	const shared_ptr<DisplayParameters>& dp=dispParams[n];
	string val;
	if(dp->getValue("OpenGLRenderer",val)){ istringstream oglre(val);
		sudodem::ObjectIO::load<decltype(renderer),cereal::XMLInputArchive>(oglre,"renderer",renderer);
	}
	else { LOG_WARN("OpenGLRenderer configuration not found in display parameters, skipped.");}
	if(dp->getValue("GLViewer",val)){ GLViewer::setState(val); displayMessage("Loaded view configuration #"+std::to_string(n)); }
...
void GLViewer::saveDisplayParameters(size_t n){
	...
	ostringstream oglre;
	sudodem::ObjectIO::save<decltype(renderer),cereal::XMLOutputArchive>(oglre,"renderer",renderer);
	dp->setValue("OpenGLRenderer",oglre.str());
	dp->setValue("GLViewer",GLViewer::getState());
	displayMessage("Saved view configuration ot #"+std::to_string(n));
}

void GLViewer::draw()
{
	// std::cerr << "DEBUG GLViewer::draw() starting" << std::endl;
	
	// Acquire lock to prevent concurrent access with simulation thread
	// std::cerr << "DEBUG GLViewer::draw() about to acquire GLLock" << std::endl;
	GLLock lock(this);
	
	// std::cerr << "DEBUG GLViewer::draw() GLLock acquired, proceeding" << std::endl;
#ifdef SUDODEM_GL2PS
	if(!nextFrameSnapshotFilename.empty() && nextFrameSnapshotFilename.ends_with(".pdf")){
	...
	if(Omega::instance().getScene()){
		...
		if(selection!=-1 && (*(Omega::instance().getScene()->bodies)).exists(selection) && isMoving){
			...
			{
				Quaternionr& r = (*(Omega::instance().getScene()->bodies))[selection]->state->ori;
		...
		/*if(manipulatedClipPlane>=0){
			...
			Se3r newSe3(Vector3r(v0,v1,v2), Quaternionr(aa.axis(), aa.angle())); newSe3.rotation.normalize();
		...
		// Update viewInfo BEFORE rendering (not in postDraw which happens after)
		...
		// std::cerr << "DEBUG GLViewer::draw() sceneRadius=" << renderer->viewInfo.sceneRadius 
		//           << " sceneCenter=" << renderer->viewInfo.sceneCenter.transpose() << std::endl;
		
		// std::cerr << "DEBUG GLViewer::draw() calling renderer->render(), scene has " << scene->bodies->size() << " bodies" << std::endl;
		renderer->render(scene, selectedName());
		// std::cerr << "DEBUG GLViewer::draw() renderer->render() completed" << std::endl;
	}
}
...
void GLViewer::postDraw(){
	...
	if(drawScale){
		...
		startScreenCoordinatesSystem();
			...
			QGLViewer::drawText(scaleCenter[0],scaleCenter[1],QString::asprintf("%.3g",(double)scaleStep));
	...
	if(timeDispMask!=0){
		...
		if(timeDispMask & GLViewer::TIME_ITER){
			ostringstream oss;
			oss<<"#"<<rb->iter;
			if(rb->stopAtIter>rb->iter) oss<<" ("<<std::setiosflags(std::ios::fixed)<<setw(3)<<setprecision(1)<<setfill('0')<<(100.*rb->iter)/rb->stopAtIter<<"%)";
			QGLViewer::drawText(x,y,oss.str().c_str());
			y-=lineHt;
		}
		if(drawGrid){
	...
	if(!nextFrameSnapshotFilename.empty()){
		#ifdef SUDODEM_GL2PS
			if(nextFrameSnapshotFilename.ends_with(".pdf")){
				gl2psEndPage();
