...
GLLock::GLLock(GLViewer* _glv): lock(Omega::instance().renderMutex), glv(_glv){
	glv->makeCurrent();
}
...
GLViewer::GLViewer(int _viewId, const shared_ptr<OpenGLRenderer>& _renderer, QGLWidget* shareWidget): QGLViewer(/*parent*/(QWidget*)NULL,shareWidget), renderer(_renderer), viewId(_viewId) {
	// std::cerr << "DEBUG GLViewer::GLViewer() starting" << std::endl;
	isMoving=false;
	drawGrid=0;
	drawScale=false;
	...
	else setWindowTitle(("Secondary view #"+std::to_string(viewId)).c_str());

	// Do NOT call show() here - the caller (sudodem.cpp) will call show()
	// after making the OpenGL context current. This ensures proper GL context
	// initialization on macOS where Qt/OpenGL must be used from the main thread.
	...
	// Note: centerScene() is NOT called here - the caller (sudodem.cpp) will call it
	// after the viewer is shown and the script has loaded bodies into the scene.
	...
	// Do NOT call centerScene() here - the caller will call it after show()
	// to ensure the scene is properly initialized
...
void GLViewer::startClipPlaneManipulation(int planeNo){
	...
	displayMessage("Manipulating clip plane #"+std::to_string(planeNo+1)+(grp.empty()?grp:" (bound planes:"+grp+")"));
}*/
...
{
	last_user_event = std::chrono::system_clock::now();
/*
	if(false){}
	// special keys: Escape and Space
		...
		if(manipulatedClipPlane>=0) {displayMessage("Clip plane #"+std::to_string(manipulatedClipPlane+1)+(renderer->clipPlaneActive[manipulatedClipPlane]?" de":" ")+"activated"); renderer->clipPlaneActive[manipulatedClipPlane]=!renderer->clipPlaneActive[manipulatedClipPlane]; }
	...
	// numbers
		...
		if(e->modifiers() & Qt::AltModifier){
			if(boundClipPlanes.count(planeId)==0) {boundClipPlanes.insert(planeId); displayMessage("Added plane #"+std::to_string(planeId+1)+" to the bound group: "+strBoundGroup());}
			else {boundClipPlanes.erase(planeId); displayMessage("Removed plane #"+std::to_string(planeId+1)+" from the bound group: "+strBoundGroup());}
		}
	...
	// letters alphabetically
		...
		if(manipulatedClipPlane>=0 && manipulatedClipPlane<renderer->numClipPlanes){
			...
			displayMessage("Plane #"+std::to_string(manipulatedClipPlane+1)+" reversed.");
		}
		else {
		...
		for(int i=0; ;i++){
			...
			if(!std::filesystem::exists(fss.str())){ nextFrameSnapshotFilename=fss.str(); break; }
		}
		LOG_INFO("Will save snapshot to "<<nextFrameSnapshotFilename);
			...
			displayMessage("Cut plane: "+std::to_string(cut_plane));
			...
			cut_plane_delta -= 1;
			displayMessage("Cut plane increment: 1e"+(cut_plane_delta>0?std::string("+"):std::string(""))+std::to_string(cut_plane_delta));
...
void GLViewer::centerPeriodic(){
	...
	float radius=std::max(std::max(halfSize[0],halfSize[1]),halfSize[2]);
...
void GLViewer::centerMedianQuartile(){
  ...
  //  nBodies +=scene->nodes->size();//including nodes from fem
	...
	for(int i=0;i<3;i++){
		...
		interQuart[i]=*(coords[i].begin()+2*nBodies/4)-*(coords[i].begin()+nBodies/4);
	}
...
void GLViewer::centerScene(){
	...
	float radius=std::max(std::max(halfSize[0],halfSize[1]),halfSize[2]); if(radius<=0) radius=1;
...
void GLViewer::postSelection(const QPoint& point)
{
	...
	if(selection>=0 && (*(Omega::instance().getScene()->bodies)).exists(selection)){
		...
		displayMessage("Selected body #"+std::to_string(selection)+(Body::byId(selection)->isClump()?" (clump)":""));
		Omega::instance().getScene()->selectedBody = selection;
			PyGILState_STATE gstate;
			gstate = PyGILState_Ensure();
      pybind11::object main=pybind11::module_::import("__main__");
      pybind11::object global=main.attr("__dict__");
				// the try/catch block must be properly nested inside PyGILState_Ensure and PyGILState_Release
				try{
          pybind11::eval<pybind11::eval_statements>(string("onBodySelect("+std::to_string(selection)+")").c_str(),global,global);
...
string GLViewer::getRealTimeString(){
	ostringstream oss;
  std::chrono::milliseconds t=Omega::instance().getRealTime_duration();
	auto total_seconds = std::chrono::duration_cast<std::chrono::seconds>(t);
	unsigned d = total_seconds.count() / 86400;
	unsigned h = (total_seconds.count() % 86400) / 3600;
	unsigned m = (total_seconds.count() % 3600) / 60;
	unsigned s = total_seconds.count() % 60;
	oss<<"clock ";
...
std::chrono::system_clock::time_point GLViewer::getLastUserEvent(){return last_user_event;};
