...
{   //std::cout<<"construct the convex hull..."<<std::endl;
    ...
    FORALLfacets
	{
		...
		// FOREACHvertex replaced
		for(size_t _vi=0; _vi < qh_setsize(vertices); _vi++){
			vertexT* vertex = ((vertexT**)vertices->e)[_vi];//traverse the vertices
...
void pybind_init__gjkparticle_utils(pybind11::module& m){
	...
	m.def("NewGJKParticle", NewGJKParticle, "Generate a GJKParticle.");
	m.def("NewGJKParticle2", NewGJKParticle2, "Generate a GJKParticle with margin.");
	m.def("GJKSphere", GJKSphere, "Generate a GJK Sphere with margin.");
	m.def("GJKPolyhedron", GJKPolyhedron, "Generate a GJK Polyhedron with margin.");
	m.def("GJKCone", GJKCone, "Generate a GJK Cone with margin.");
	m.def("GJKCylinder", GJKCylinder, "Generate a GJK Cylinder with margin.");
	m.def("GJKCuboid", GJKCuboid, "Generate a GJK Cuboid with margin.");
}
