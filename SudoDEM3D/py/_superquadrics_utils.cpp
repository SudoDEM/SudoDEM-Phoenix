...
// Uses std::random for pseudo-random sequences.
// For true quasi-random sequences (Sobol), consider using a dedicated library.
...
inline int Sign(double f){ if(f<0) return -1; if(f>0) return 1; return 0; }
...
std::vector<double> MonteCarlo_test(Vector6r rxyz, Vector2r eps,long int num_max = 1000000){
  ...
  //pseudo-random numbers using std::random
  // NOTE: For true quasi-random sequences (Sobol), a dedicated library would be needed.
  // This implementation uses std::mt19937.
  static std::mt19937 gen(std::random_device{}());
  static std::uniform_real_distribution<> dist(0,1);

  for(;num<=num_max;num++){
    x = x_l*dist(gen)-rxyz(1);
    y = y_l*dist(gen)-rxyz(3);
    z = z_l*dist(gen)-rxyz(5);
	...
	for ( ; bi!=biEnd; ++bi )
	{
		...
		if (b->shape->getClassName()=="Superquadrics"){
		    ...
		    for(a=0.0,i=0;i<h;i++,a+=hStep)
		      for(b=0.0,j=0;j<w;j++,b+=wStep)
		      {
                Vector3r Surf = A->getSurface(Vector2r(b,a-Mathr::HALF_PI));
      ...
      point_counter ++;
		...
		//	State* state=b->state.get();
		    ...
		    for(a=0.0,i=0;i<h;i++,a+=hStep)
		      for(b=0.0,j=0;j<w;j++,b+=wStep)
		      {
                Vector3r Surf = getSurface_PolySuperEllipsoid2(Vector2r(b, a-Mathr::HALF_PI), R,eps,pShape);
      ...
      point_counter ++;
		...
		//	State* state=b->state.get();
		    ...
		    for(a=0.0,i=0;i<h;i++,a+=hStep)
		      for(b=0.0,j=0;j<w;j++,b+=wStep)
		      {
                Vector3r Surf = getSurface_PolySuperEllipsoid(Vector2r(b, a-Mathr::HALF_PI), R,eps,pShape);
      ...
      point_counter ++;
		...
		//	State* state=b->state.get();
		    ...
		    for(a=0.0,i=0;i<h;i++,a+=hStep)
		      for(b=0.0,j=0;j<w;j++,b+=wStep)
		      {
                Vector3r Surf = getSurface_PolyEllipsoid(b, a-Mathr::HALF_PI, R,pShape);
...
void pybind_init__superquadrics_utils(pybind11::module& m){
    ...
    m.def("Superquadrics_test",Superquadrics_test,"test the computation of points on a superquadric given a normal vector.");
    m.def("PositionPB",PositionPB,"generate a pair of particles for test.");
    m.def("EST_Support",EST_Support,"Estimate the time consumption of the support function for a particle. For test only.");
    //m.def("MonteCarlo_test",MonteCarlo_test,(pybind11::arg("num_max")=1000000),"test the computation of particle geometric quantities (volume, mass center, and moment of inertia) by MonteCarlo simulation.");
    m.def("outputWalls", outputWalls, "output positions of walls.");
    m.def("getSurfArea", getSurfArea, "getSurfArea(id,w,h): get the surface area of a super-ellipsoid by a given particle id with resolution w and h (e.g., w=10,h=10). Larger w and h will yield more accurate results.");
    m.def("outputParticles", outputParticles, "output particles.");
    m.def("outputParticlesbyIds", outputParticlesbyIds, "output particles by a list of ids.");
	  m.def("NewSuperquadrics", NewSuperquadrics, "Generate a Superquadric.");
    m.def("NewPolySuperellipsoid", NewPolySuperellipsoid, "Generate a PolySuperellipsoid.");
    m.def("NewPolySuperellipsoid_rot", NewPolySuperellipsoid_rot, "Generate a PolySuperellipsoid with specified quaternion components: q0(w),q1(x),q2(y),q3(z)..");
	  m.def("NewSuperquadrics2", NewSuperquadrics2, "Generate a Superquadric with isSphere option.");
	  m.def("NewSuperquadrics_rot", NewSuperquadrics_rot, "Generate a Super-ellipsoid with specified quaternion components: q0(w),q1(x),q2(y),q3(z).");
    m.def("NewSuperquadrics_rot2", NewSuperquadrics_rot2, "Generate a Super-ellipsoid with specified quaternion components: q0(w),q1(x),q2(y),q3(z) with isSphere option.");
	  m.def("outputPOV",outputPOV,"output Superquadrics into a POV file for post-processing using POV-ray.");
    m.def("PolySuperellipsoidPOV",PolySuperellipsoidPOV,"output PolySuperellipsoids into a POV file for post-processing using POV-ray.");
    m.def("outputVTK",outputVTK,"output Superquadrics into a VTK file for post-processing using Paraview.");
    m.def("outputVTK_polyEllipsoid",outputVTK_polyEllipsoid,"output Poly-ellipsoid into a VTK file for post-processing using Paraview.[Only for test.]");
    m.def("outputVTK_polySuperEllipsoid",outputVTK_PolySuperEllipsoid,"output Poly-Superellipsoid into a VTK file for post-processing using Paraview.[Only for test.]");
    m.def("outputVTK_polySuperEllipsoid2",outputVTK_PolySuperEllipsoid2,"output Poly-Superellipsoid into a VTK file for post-processing using Paraview.[Only for test.]");
}
