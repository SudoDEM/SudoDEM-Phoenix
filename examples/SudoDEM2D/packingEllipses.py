...
ground_wall = utils.wall(-0.5*boxsize[1],axis=1,sense=1, material = 'mat1')#ground
O.bodies.append(ground_wall)
left_wall = utils.wall(-0.5*boxsize[0],axis=0,sense=1, material = 'mat1')#left
O.bodies.append(left_wall)
right_wall = utils.wall(0.5*boxsize[0],axis=0,sense=-1, material = 'mat1')#right
...
O.dt = 1e-3
Addlayer(r,mat,boxsize,num_s,num_t)
newton=NewtonIntegrator(); newton.damping=0.1; newton.gravity=Vector2(0.,-10.0); newton.isSuperellipse=True
...
print(f"Total bodies in scene: {len(O.bodies)}")
